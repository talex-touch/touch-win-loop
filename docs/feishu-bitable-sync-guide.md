...
4. 如果你希望事件增量同步生效，还需要把事件回调配置到：
   ...
   - 并在飞书侧订阅 `drive.file.bitable_record_changed_v1`
5. 管理员账号具备平台权限：
   - 飞书配置：`role.assign`
...
- 这意味着你可以先调整映射、同步选项、自动同步规则、回填列，再立即看预检结果。
- 调度不在子表同步项里配置，而是在“多维同步信息”这一层统一配置。
- 映射区会把当前实体支持的每个目标字段都展开成独立配置行，不需要一条条手工新增。
- 关键字段会直接标 `重点`，上方也会提示当前还缺哪些关键映射。
- 如果旧同步项里混进了别的实体类型字段，可以点“整理为当前实体模板”快速清干净，再继续配。
- 回填配置里的字段会直接从当前飞书字段列表中下拉选择，不需要手动输入列名。
- 预检结果里会显示“模拟同步结果”，方便你直接确认竞赛库、赛道库最终会同步成什么样。
- 新增同步项时会按子表/视图名称给出 `contest / track / resource` 识别建议，并生成推荐名称；不会再静默改掉你手动选的类型。
...
含义：飞书事件到达时，如何根据记录状态和字段变化自动决定“直接同步”还是“先重置状态”。
...
- `同步信息 = 未同步`
  说明：自动触发当前记录的增量同步
- `记录状态 = 已完成` 且 `同步信息 = 已同步`
  说明：如果业务字段被修改，自动改回 `记录状态 = 撰写中`、`同步信息 = 未同步`
...
如果你的表里字段名不同，也可以在自动同步规则里改成自己的列名。

...
log() {
  printf '[winloop-webhook] %s\n' "$*"
}

error() {
  printf '[winloop-webhook][error] %s\n' "$*" >&2
}

usage() {
  cat <<'USAGE'
Usage:
  deploy-winloop-1panel-webhook.sh [options]
...
Options:
  ...
  --default-image <repo>        Default image repo, for example ghcr.io/acme/touch-win-loop
  --default-tag <tag>           Default image tag (default: latest)
  --default-image-ref <value>   Default full image ref (repo:tag or repo@digest)
  --allowed-branch <name>       Allowed branch (default: main)
  --allowed-repository <name>   Allowed repository (for example owner/touch-win-loop)
...
Environment fallback:
  WINLOOP_WEBHOOK_PAYLOAD_JSON
  WINLOOP_WEBHOOK_REQUEST_TOKEN
  WINLOOP_WEBHOOK_TOKEN
  WINLOOP_WEBHOOK_DEPLOY_SCRIPT
  WINLOOP_WEBHOOK_DEFAULT_IMAGE
  WINLOOP_WEBHOOK_DEFAULT_TAG
...
PAYLOAD_FILE=""
PAYLOAD_JSON="${WINLOOP_WEBHOOK_PAYLOAD_JSON:-}"
REQUEST_TOKEN="${WINLOOP_WEBHOOK_REQUEST_TOKEN:-}"
EXPECTED_TOKEN="${WINLOOP_WEBHOOK_TOKEN:-}"
DEPLOY_SCRIPT="${WINLOOP_WEBHOOK_DEPLOY_SCRIPT:-${SCRIPT_DIR}/deploy-winloop-1panel.sh}"
DEFAULT_IMAGE="${WINLOOP_WEBHOOK_DEFAULT_IMAGE:-${WINLOOP_IMAGE_REPO:-}}"
DEFAULT_TAG="${WINLOOP_WEBHOOK_DEFAULT_TAG:-${WINLOOP_IMAGE_TAG:-latest}}"
DEFAULT_IMAGE_REF="${WINLOOP_WEBHOOK_DEFAULT_IMAGE_REF:-${WINLOOP_IMAGE_REF:-}}"
ALLOWED_BRANCH="${WINLOOP_WEBHOOK_ALLOWED_BRANCH:-main}"
ALLOWED_REPOSITORY="${WINLOOP_WEBHOOK_ALLOWED_REPOSITORY:-}"
OVERRIDE_SHA=""
OVERRIDE_TAG=""
OVERRIDE_IMAGE=""
OVERRIDE_IMAGE_REF=""
...
if [[ -n "$resolved_image_ref" ]]; then
  export WINLOOP_IMAGE_REF="$resolved_image_ref"
  unset WINLOOP_IMAGE_REPO
  unset WINLOOP_IMAGE_TAG
  log "Resolved deploy image ref: $resolved_image_ref"
else
  export WINLOOP_IMAGE_REPO="$resolved_image"

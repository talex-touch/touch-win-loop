...
- [Intelligence Workflow v1 项目内工作流方案](./docs/intelligence-workflow-v1.md)

## Intelligence Workflow v1

项目工作台中的 `Loopy 数据 -> 智能工作流` 已接入第一版项目内 workflow 能力，当前边界如下：

- 工作流定义、run 历史、run step 历史全部走 PostgreSQL 持久化。
- 工作流步骤只支持 `prompt / tool / agent` 三类。
- 默认只做项目内上下文，不引入桌面 clipboard / 前台应用 / secure-store 语义。
- 写操作继续复用 `ai_project_change_requests`，工作流 run 会在 `needs_review` 暂停，审批完成后再 `continue`。
- 外部桥接当前先只做 provider tool bridge，内置 `feishu_bitable` / `coze_workflow` 两类 provider tool key。

详细设计、工具目录和审批边界见：

- [docs/intelligence-workflow-v1.md](./docs/intelligence-workflow-v1.md)

...
it('项目页头部工作台 tabs 与右栏答辩模式共用同一套状态', async () => {
  ...
  assert.match(workspaceSource, /if \(nextMode === 'final_review'\) \{/, '项目页缺少终审工作台切换分支')
  assert.match(workspaceSource, /workbenchMode: normalizeProjectWorkbenchMode\(source\.workbenchMode\)/, '项目页未统一归一化终审工作台状态')
  assert.match(workspaceSource, /if \(next === 'defense'\) \{\s+workbenchMode\.value = 'defense'/, 'aiMode 切换到答辩时未同步工作台 tabs')
  assert.match(workspaceSource, /if \(workbenchMode\.value !== 'final_review'\)\s+workbenchMode\.value = 'project'/, '项目页在终审工作台下仍会被普通 AI 模式切回项目工作台')

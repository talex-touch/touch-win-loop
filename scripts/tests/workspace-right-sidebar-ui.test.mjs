...
const WORKSPACE_TAB_LAYOUT_UTIL_FILE = resolve(process.cwd(), 'shared/utils/workspace-tab-layout.ts')
const PROJECT_VIEW_STORE_FILE = resolve(process.cwd(), 'server/utils/project-workspace-view-store.ts')
...
it('右栏在 AgentProto contextual 场景提供统一操作条，并把 workflow / scene 草案事件透传给项目页', async () => {
  const [sidebarSource, workspaceSource] = await Promise.all([
    readFile(RIGHT_SIDEBAR_FILE, 'utf8'),
    readFile(WORKSPACE_DETAIL_FILE, 'utf8'),
  ])

  assert.match(sidebarSource, /projectContextualAssistantKey\?: WorkspaceContextualAssistantKey \| ''/, '右栏缺少 contextual assistant key 入参')
  ...
  assert.match(sidebarSource, /sceneResourceId\?: string/, '右栏缺少当前 scene 资源入参')
  assert.match(sidebarSource, /sceneHash\?: string/, '右栏缺少当前 scene hash 入参')
  assert.match(sidebarSource, /appliedSceneDraftKeys\?: string\[\]/, '右栏缺少 scene 已应用草案 key 入参')
  assert.match(sidebarSource, /discardedSceneDraftKeys\?: string\[\]/, '右栏缺少 scene 已丢弃草案 key 入参')
  assert.match(sidebarSource, /sceneGenerateAvailable\?: boolean/, '右栏缺少 scene 生成动作可用态入参')
  assert.match(sidebarSource, /sceneGenerateDisabledReason\?: string/, '右栏缺少 scene 生成动作禁用原因入参')
  ...
  assert.match(sidebarSource, /'requestSceneDraft': \[payload:/, '右栏缺少 scene draft 请求事件')
  assert.match(sidebarSource, /'applySceneDraft': \[draft: AiWorkspaceSceneDraft\]/, '右栏缺少 scene apply 事件')
  assert.match(sidebarSource, /'discardSceneDraft': \[draft: AiWorkspaceSceneDraft\]/, '右栏缺少 scene discard 事件')
  ...
  assert.match(sidebarSource, /const showAgentProtoControls = computed\(\(\) => \{/, '右栏未按 contextual assistant key 统一判断 AgentProto 工具栏')
  assert.match(sidebarSource, /const isAgentProtoWorkflowContext = computed\(\(\) => showAgentProtoControls\.value && Boolean\(String\(props\.workflowResourceId \|\| ''\)\.trim\(\)\)\)/, '右栏未区分 AgentProto workflow 上下文')
  assert.match(sidebarSource, /const isAgentProtoSceneContext = computed\(\(\) => showAgentProtoControls\.value && !isAgentProtoWorkflowContext\.value && Boolean\(String\(props\.sceneResourceId \|\| ''\)\.trim\(\)\)\)/, '右栏未区分 AgentProto scene 上下文')
  assert.match(sidebarSource, /function isAgentProtoActionDisabled\(action: WorkflowDraftAction\): boolean \{/, '右栏缺少统一的 AgentProto 动作禁用态判断')
  assert.match(sidebarSource, /function requestAgentProtoDraft\(action: WorkflowDraftAction\): void \{[\s\S]*requestWorkflowDraft\(action\)[\s\S]*requestSceneDraft\(action\)/, '右栏未按上下文路由到 workflow / scene 草案请求')
  ...
  assert.match(sidebarSource, /:disabled="isAgentProtoActionDisabled\('generate'\)"/, '右栏 AgentProto 生成按钮未接入统一禁用态')
  assert.match(sidebarSource, /:disabled="isAgentProtoActionDisabled\('complete'\)"/, '右栏 AgentProto 补全按钮未接入统一禁用态')
  assert.match(sidebarSource, /:disabled="isAgentProtoActionDisabled\('refine'\)"/, '右栏 AgentProto 续改按钮未接入统一禁用态')
  assert.match(sidebarSource, /:disabled="isAgentProtoActionDisabled\('restyle'\)"/, '右栏 AgentProto 调样式按钮未接入统一禁用态')
  ...
  assert.match(sidebarSource, /应用到当前自由画布/, '右栏缺少 scene 草案应用按钮')
  assert.match(sidebarSource, /requestApplySceneDraft/, '右栏缺少 scene 草案应用入口')
  assert.match(sidebarSource, /requestDiscardSceneDraft/, '右栏缺少 scene 草案丢弃入口')
  assert.match(sidebarSource, /系统上下文/, '右栏仍未把架构视图枚举映射成中文')
  assert.match(sidebarSource, /从左到右/, '右栏仍未把布局枚举映射成中文')
  assert.match(sidebarSource, /默认/, '右栏仍未把样式枚举映射成中文')
  ...
  assert.match(workspaceSource, /:project-contextual-assistant-key="projectContextualAssistant\?\.key \|\| ''"/, '项目页未向右栏透传 contextual assistant key')
  assert.match(workspaceSource, /:scene-resource-id="activeAgentProtoSceneResourceId"/, '项目页未向右栏透传 scene 资源 id')
  assert.match(workspaceSource, /:scene-hash="activeAgentProtoSceneHash"/, '项目页未向右栏透传 scene hash')
  assert.match(workspaceSource, /:scene-generate-available="sceneGenerateAvailable"/, '项目页未向右栏透传 scene 生成动作可用态')
  assert.match(workspaceSource, /:scene-generate-disabled-reason="sceneGenerateDisabledReason"/, '项目页未向右栏透传 scene 生成动作禁用原因')
  ...
  assert.match(workspaceSource, /@request-scene-draft="requestSceneDraftFromSidebar"/, '项目页未接入右栏 scene draft 请求事件')
  assert.match(workspaceSource, /@apply-scene-draft="applySceneDraft"/, '项目页未接入右栏 scene apply 事件')
  assert.match(workspaceSource, /@discard-scene-draft="discardSceneDraft"/, '项目页未接入右栏 scene discard 事件')
})
...
it('项目页会在切换会话与恢复历史消息时清空 workflow 草案本地状态，并按动作维度计算 AgentProto gating', async () => {
  ...
  assert.match(source, /function resetChatDraftArtifactState\(\): void \{[\s\S]*appliedAgentDocDraftKeys\.value = \[\][\s\S]*appliedWorkflowDraftKeys\.value = \[\][\s\S]*discardedWorkflowDraftKeys\.value = \[\][\s\S]*appliedSceneDraftKeys\.value = \[\][\s\S]*discardedSceneDraftKeys\.value = \[\][\s\S]*\}/, '项目页缺少统一的草案本地状态清理入口')
  ...
  assert.match(source, /const sceneGenerateAvailable = computed\(\(\) => !sceneCanvasUnavailableReason\.value && isAiFeatureAvailable\(resolveWorkflowDraftFeatureKey\('generate'\)\)\)/, '项目页缺少 scene 生成动作可用性计算')
  assert.match(source, /const sceneCompleteAvailable = computed\(\(\) => !sceneCanvasUnavailableReason\.value && isAiFeatureAvailable\(resolveWorkflowDraftFeatureKey\('complete'\)\)\)/, '项目页缺少 scene 补全动作可用性计算')
  ...
  assert.match(source, /async function requestSceneDraftFromSidebar\(options: SceneDraftRequestOptions\): Promise<void> \{[\s\S]*if \(!ensureAiFeatureAvailable\(resolveWorkflowDraftFeatureKey\(options\.action\)\)\)\s+return/, '项目页 scene 草案请求入口未按动作 feature 前置 gating')
})
...
it('右栏将 system 消息渲染为内联消息，并将 assistant 内容接到统一引用渲染组件', async () => {
  ...
  assert.match(sidebarSource, /const shouldAnimateNewChatEntries = ref\(false\)/, '右栏缺少新消息动画门控状态')
  assert.match(sidebarSource, /markVisibleChatEntriesAsSeen\(\)/, '右栏缺少历史消息静态种子逻辑')
  assert.match(sidebarSource, /workspace-chat-entry--entering/, '右栏缺少新消息入场动画类')
  assert.match(sidebarSource, /workspace-chat-entry--recent-user/, '右栏缺少用户消息一次性高亮态')
  assert.match(sidebarSource, /workspace-chat-entry--streaming-assistant/, '右栏缺少 assistant 流式态样式')
  assert.match(sidebarSource, /workspace-chat-assistant-stream 1\.5s ease-in-out infinite/, '右栏 assistant 流式态缺少动态边条动效')
  assert.match(sidebarSource, /@media \(prefers-reduced-motion: reduce\)/, '右栏缺少 reduced motion 降级处理')
...
it('右栏消息区会在贴底时自动跟随，并在切换会话后回到底部', async () => {
  ...
  assert.match(source, /watch\(displayedActiveChatSessionId, async \(nextId, previousId\) => \{[\s\S]*chatShouldStickToBottom\.value = true[\s\S]*scrollChatViewportToBottom\(true\)/, '右栏切换会话后未强制回到底部')
...
it('右栏会话区改为横向 tabs，右侧常驻新建对话并按类型显示图标', async () => {
  const [source, workspaceSource, shellSource, storeSource, tabLayoutSource] = await Promise.all([
    readFile(RIGHT_SIDEBAR_FILE, 'utf8'),
    readFile(WORKSPACE_DETAIL_FILE, 'utf8'),
    readFile(WORKSPACE_PROJECT_SHELL_FILE, 'utf8'),
    readFile(PROJECT_VIEW_STORE_FILE, 'utf8'),
    readFile(WORKSPACE_TAB_LAYOUT_UTIL_FILE, 'utf8'),
  ])

  assert.match(source, /openChatSessionIds\?: string\[\]/, '右栏缺少打开中的会话 tabs id 列表入参')
  assert.match(source, /tabSpacingPreset\?: WorkspaceTabSpacingPreset \| ''/, '右栏缺少 tab 间距预设入参')
  assert.match(source, /const openChatSessions = computed\(\(\) => \{/, '右栏缺少基于打开中会话 id 的 tabs 计算逻辑')
  assert.match(source, /const workspaceRightSessionTabLayoutStyle = computed<Record<string, string>>\(\(\) => \{/, '右栏缺少基于统一密度的 tabs 样式计算')
  assert.match(source, /:style="workspaceRightSessionTabLayoutStyle"/, '右栏未将 tabs 密度样式挂到根节点')
  ...
  assert.doesNotMatch(source, /<a-trigger[\s\S]*trigger="hover"[\s\S]*workspace-right-sidebar__session-popover/, '右栏会话 tabs 仍保留难用的 hover 详情浮层')
  ...
  assert.match(source, /function resolveWorkspaceRightSessionTabMetric\(value: string, minimum: number\): string \{/, '右栏会话 tabs 缺少右栏保底尺寸逻辑')
  ...
  assert.match(source, /'--workspace-right-session-strip-height': resolveWorkspaceRightSessionTabMetric\(density\.stripHeight, 40\)/, '右栏会话条高度未设置保底尺寸')
  assert.match(source, /'--workspace-right-session-tab-min-width': resolveWorkspaceRightSessionTabMetric\(density\.minWidth, 144\)/, '右栏会话 tabs 最小宽度未设置保底尺寸')
  assert.match(source, /\.workspace-right-sidebar__session-strip \{[\s\S]*height:\s*var\(--workspace-right-session-strip-height, 40px\)/, '右栏会话条高度仍未改为统一密度变量')
  assert.match(source, /\.workspace-right-sidebar__session-tab \{[\s\S]*min-width:\s*var\(--workspace-right-session-tab-min-width, 132px\)[\s\S]*padding:\s*0 var\(--workspace-right-session-tab-padding-x, var\(--workspace-right-space-3_5\)\)/, '右栏会话 tabs 仍未改为统一密度变量')
  assert.match(source, /\.workspace-right-sidebar__session-tab-label \{[\s\S]*font-size:\s*var\(--workspace-right-session-tab-label-size, var\(--workspace-right-font-xs\)\)/, '右栏会话 tabs 标签字号仍未改为统一密度变量')
  assert.match(source, /\.workspace-right-sidebar__session-action \{[\s\S]*width:\s*var\(--workspace-right-session-action-size, 40px\)/, '右栏会话操作按钮宽度仍未改为统一密度变量')
  ...
  assert.match(source, /:title="resolveSessionTitle\(session\)"/, '右栏会话 tabs 未保留原生标题提示')
  assert.doesNotMatch(source, /data-testid="workspace-right-sidebar-session-popover"/, '右栏会话 tabs 仍保留难用的 hover 弹框')
  assert.doesNotMatch(source, /trigger="hover"\s+position="bottom"/, '右栏会话 tabs 仍通过 hover trigger 渲染浮层')
  ...
  assert.match(workspaceSource, /:tab-spacing-preset="workspaceEffectiveTabSpacingPreset"/, '项目页未向右栏透传统一的 tab 间距预设')
  assert.match(workspaceSource, /defaultRightSidebarWidth: workspaceEffectiveDefaultRightSidebarWidth,[\s\S]*projectAssistantMode,[\s\S]*rightSidebarView,[\s\S]*setRightSidebarUserCollapsed,/, '项目页未将右栏视图态接入工作区视图状态')
  assert.match(workspaceSource, /activeMeetingId,[\s\S]*projectAssistantMode,[\s\S]*rightSidebarView,[\s\S]*leftSidebarWidth,/, '项目页未监听右栏视图态变化来同步工作区视图')
  assert.match(tabLayoutSource, /export function resolveWorkspaceTabDensityTokens\(/, '缺少共享的工作区 tab 密度解析工具')
  assert.match(shellSource, /projectAssistantMode: options\.projectAssistantMode\.value/, '工作区视图状态未持久化项目助手模式')
  assert.match(shellSource, /rightSidebarView: options\.rightSidebarView\.value/, '工作区视图状态未持久化右栏视图')
  assert.match(storeSource, /projectAssistantMode: normalizeProjectWorkspaceAssistantMode\(source\.projectAssistantMode\)/, '后端未保存项目助手模式')
  assert.match(storeSource, /rightSidebarView: normalizeProjectWorkspaceRightSidebarView\(source\.rightSidebarView\)/, '后端未保存右栏视图')
})
...
it('右侧 AI 历史 tabs 会作为用户项目级共享状态跨设备同步，并在本地空态时回落恢复', async () => {
  const [workspaceSource, shellSource, storeSource, schemaSource] = await Promise.all([
    readFile(WORKSPACE_DETAIL_FILE, 'utf8'),
    readFile(WORKSPACE_PROJECT_SHELL_FILE, 'utf8'),
    readFile(PROJECT_VIEW_STORE_FILE, 'utf8'),
  ...
  assert.match(schemaSource, /CREATE TABLE IF NOT EXISTS user_project_workspace_ai_tabs \(/, '缺少用户项目级 AI tabs 共享状态表')
  assert.match(schemaSource, /payload JSONB NOT NULL/, '用户项目级 AI tabs 共享状态未使用 JSONB 存储')
  assert.match(schemaSource, /idx_user_project_workspace_ai_tabs_project_user/, '缺少用户项目级 AI tabs 共享状态索引')

  assert.match(storeSource, /export function normalizeProjectWorkspaceAiTabsStatePayload\(/, '后端未抽出 AI tabs 共享状态归一化逻辑')
  assert.match(storeSource, /function mapProjectWorkspaceAiTabs\(/, '后端未提供 AI tabs 共享状态映射逻辑')
  assert.match(storeSource, /FROM user_project_workspace_ai_tabs/, '后端未读取用户项目级 AI tabs 共享状态表')
  assert.match(storeSource, /INSERT INTO user_project_workspace_ai_tabs \(/, '后端未写入用户项目级 AI tabs 共享状态表')
  assert.match(storeSource, /DELETE FROM user_project_workspace_ai_tabs/, '后端未支持清空用户项目级 AI tabs 共享状态')
  assert.match(storeSource, /personalAiTabs,/, '研发工作台视图 GET 未返回共享 AI tabs 状态')
  assert.match(storeSource, /await upsertUserProjectWorkspaceAiTabs\(db, user\.id, normalizedProjectId, \{[\s\S]*openChatSessionIds: normalizedPayload\.openChatSessionIds,[\s\S]*activeChatSessionId: normalizedPayload\.activeChatSessionId,[\s\S]*\}\)/, '研发工作台视图写入时未同步共享 AI tabs 状态')

  assert.match(shellSource, /function applyPersonalAiTabsFallback\(/, '前端未抽出共享 AI tabs 的空态回落逻辑')
  assert.match(shellSource, /bundle\.personalAiTabs/, '前端 hydrate 未消费共享 AI tabs 状态')
  assert.match(shellSource, /openChatSessionIds: personalAiTabs\.payload\.openChatSessionIds/, '前端空态回落时未恢复共享 AI tabs 打开列表')
  assert.match(shellSource, /activeChatSessionId: personalAiTabs\.payload\.activeChatSessionId/, '前端空态回落时未恢复共享 AI tabs 当前会话')
  assert.match(workspaceSource, /personalAiTabs\?: ProjectWorkspaceAiTabsPreference \| null/, '项目页未感知研发工作台视图里的共享 AI tabs 结构')
})
...
it('项目页头部工作台改为研发/答辩/终审分流，右栏助手按工作台解耦', async () => {
  ...
  assert.match(workspaceSource, /const projectContextualAssistant = computed<WorkspaceProjectContextualAssistant \| null>\(\(\) => \{[\s\S]*if \(previewMode\.value === 'markdown'\) \{[\s\S]*key: 'agent_doc'[\s\S]*label: 'AgentDoc'[\s\S]*if \(activePreviewResourcePurpose\.value === 'design'\) \{[\s\S]*key: 'design_assistant'[\s\S]*label: '设计助手'[\s\S]*if \(activePreviewResourcePurpose\.value === 'workflow'\) \{[\s\S]*key: 'agent_proto'[\s\S]*label: 'AgentProto'[\s\S]*return \{[\s\S]*key: 'agent_proto'[\s\S]*label: 'AgentProto'/, '项目页未按 tab 类型解析 AgentDoc / 设计助手 / AgentProto contextual key')
  ...
  assert.match(rightSidebarSource, /当前为设计助手，可以先聊结构、层级和交互；需要落地时再生成待确认结构源草案。/, '右栏缺少设计助手 contextual 自由聊天提示语')
  assert.match(rightSidebarSource, /当前为 AgentProto，可以先聊想法，也可以用上方动作生成待确认草案；应用前会再次确认。/, '右栏缺少 AgentProto contextual 自由聊天提示语')
...
it('右栏空会话展示按 AI 场景区分的真实感 mock，并在真实消息出现后让位', async () => {
  const source = await readFile(RIGHT_SIDEBAR_FILE, 'utf8')

  assert.match(source, /function buildMockChatSessions\(\): AiChatSession\[\] \{/, '右栏缺少空会话 mock 会话构造函数')
  assert.match(source, /function buildMockChatMessages\(threadKey: MockChatThreadKey = 'current'\): ChatMessage\[\] \{/, '右栏缺少空会话 mock 消息构造函数')
  assert.match(source, /if \(props\.chatMessages\.length > 0 \|\| props\.chatMessagesLoading \|\| props\.workspacePreparing \|\| !props\.aiEnabled\)\s+return props\.chatMessages/, '右栏 mock 未在真实消息、加载态或禁用态让位')
  assert.match(source, /const visibleChatSessions = computed\(\(\) => props\.chatSessions\.length > 0 \? props\.chatSessions : mockChatSessions\.value\)/, '右栏历史记录未在无真实会话时接入 mock 会话')
  assert.match(source, /return buildMockChatMessages\(resolveMockChatThreadKey\(displayedActiveChatSessionId\.value\)\)/, '右栏空会话未按会话选择接入 mock 消息')
  assert.match(source, /mockChatSessions\.value\.slice\(0, 4\)\.map\(session => session\.id\)/, '右栏打开会话 tab 未展示 mock 会话')
  assert.match(source, /if \(isMockChatSessionId\(sessionId\)\) \{[\s\S]*selectedMockChatSessionId\.value = sessionId[\s\S]*return[\s\S]*\}/, '右栏 mock 会话点击不应触发真实切换请求')
  assert.match(source, /props\.chatSessionDeletingId === session\.id \|\| isMockChatSessionId\(session\.id\)/, '右栏 mock 会话不应允许删除')
  assert.match(source, /messageCount: 12/, '右栏 mock 会话记录不够丰富')
  assert.match(source, /我看了一下当前设计稿，主要问题不在视觉细节/, '设计助手缺少真实感 mock 对话')
  assert.match(source, /我已进入 AgentProto 上下文/, 'AgentProto 缺少真实感 mock 对话')
  assert.match(source, /AgentDef 已按答辩模式准备好/, 'AgentDef 缺少真实感 mock 对话')
  assert.match(source, /我会只生成待审批提案，不会自动修改项目/, '自动优化缺少审批语义 mock')
  assert.match(source, /我会按评分映射、证据链、量化指标和资料完整度四个维度检查/, '寻疑发现缺少真实扫描语义 mock')
  assert.match(source, /我先按终审视角过了一遍当前材料/, '终审助手缺少真实感 mock 对话')
  assert.match(source, /当前改动建议只补 mock，不改布局尺寸，风险最小。/, '设计助手 mock 消息不足 8 条')
  assert.match(source, /如果你点上方“AI 生成”，我会按横向流程图产出待确认草案。/, 'AgentProto mock 消息不足 8 条')
  assert.match(source, /下一轮我建议模拟“评委要求现场证明引用来源”的场景。/, 'AgentDef mock 消息不足 8 条')
})

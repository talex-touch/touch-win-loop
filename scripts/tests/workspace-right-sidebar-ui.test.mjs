...
it('项目页头部工作台改为研发/答辩/终审分流，右栏助手按工作台解耦', async () => {
  ...
  assert.match(rightSidebarSource, /当前为设计助手，可生成结构源草案；确认后再导入到当前设计内容，不会静默覆盖。/, '右栏缺少设计助手 contextual 提示语')
  assert.match(rightSidebarSource, /当前为 AgentProto，可生成、补全、续改或调样式；结果会先产出草案，确认后才会应用到当前画布。/, '右栏缺少 AgentProto contextual 提示语')

...
it('右栏在 AI 未配置时展示禁用提示，并禁用当前模式输入与触发按钮', async () => {
  ...
  assert.match(sidebarSource, /const documentAssistComposerPlaceholder = computed\(\(\) => \{/, '右栏缺少文稿助手输入框占位逻辑')
  assert.match(sidebarSource, /workspace-chat-composer__footer--doc/, '右栏文稿助手未复用原有输入框样式')
  assert.match(sidebarSource, /:disabled="documentAssistSendDisabled"/, '右栏文稿助手发送按钮未接入禁用态')

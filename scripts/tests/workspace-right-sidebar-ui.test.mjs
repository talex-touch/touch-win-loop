...
it('项目页通过头部 AI 按钮切换右栏或终审助手，并清理旧 hover handle 与浮动按钮', async () => {
  ...
  assert.match(workspaceSource, /function toggleRightSidebar\(\): void \{[\s\S]*if \(workbenchMode\.value === 'final_review'\) \{[\s\S]*toggleFinalReviewAssistantDrawer\(\)[\s\S]*return[\s\S]*\}/, '项目页未在终审工作台下把 header AI 按钮重定向到终审助手抽屉')
...
it('项目页头部工作台 tabs 与答辩模式仍共用状态，终审工作台改走独立驾驶舱', async () => {
  ...
  assert.match(workspaceSource, /const finalReviewMaterialsOpen = ref\(false\)/, '项目页缺少终审资料抽屉状态')
  assert.match(workspaceSource, /const finalReviewAssistantOpen = ref\(false\)/, '项目页缺少终审助手抽屉状态')

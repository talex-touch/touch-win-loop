...
it('项目页会在切换会话与恢复历史消息时清空 workflow 草案本地状态，并按动作维度计算 AgentProto gating', async () => {
  ...
  assert.match(source, /const workflowGenerateAvailable = computed\(\(\) => isAiFeatureAvailable\(resolveWorkflowDraftFeatureKey\('generate'\)\)\)/, '项目页缺少 workflow 生成动作可用性计算')
  assert.match(source, /const workflowCompleteAvailable = computed\(\(\) => isAiFeatureAvailable\(resolveWorkflowDraftFeatureKey\('complete'\)\)\)/, '项目页缺少 workflow 补全动作可用性计算')
  assert.match(source, /const workflowRefineAvailable = computed\(\(\) => isAiFeatureAvailable\(resolveWorkflowDraftFeatureKey\('refine'\)\)\)/, '项目页缺少 workflow 续改动作可用性计算')
  assert.match(source, /const workflowRestyleAvailable = computed\(\(\) => isAiFeatureAvailable\(resolveWorkflowDraftFeatureKey\('restyle'\)\)\)/, '项目页缺少 workflow 调样式动作可用性计算')
...
it('右栏将 system 消息渲染为步骤卡，并将 assistant 内容接到统一引用渲染组件', async () => {
  const [sidebarSource, workspaceSource] = await Promise.all([
    readFile(RIGHT_SIDEBAR_FILE, 'utf8'),
    readFile(WORKSPACE_DETAIL_FILE, 'utf8'),
  ])
  ...
  assert.match(sidebarSource, /workspace-chat-system-card/, '右栏缺少流式步骤卡样式')
  assert.match(sidebarSource, /data-testid="workspace-chat-system-card"/, '右栏缺少步骤卡测试锚点')

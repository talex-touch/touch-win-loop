...
it('独立设备排布编辑器提供模板、手动 transform、批量导出与截图资产化', async () => {
  ...
  assert.match(devicePanel, /data-testid="workspace-device-arrangement-workbench"/, '独立设备排布编辑器缺少专用工作台布局')
  assert.match(devicePanel, /grid-template-columns: 288px minmax\(0, 1fr\) 320px/, '设备排布工作台未保持稳定三栏布局')
  ...
  assert.match(devicePanel, /data-testid="workspace-device-arrangement-dirty-state"/, '独立设备排布编辑器缺少 dirty\/save 状态')
  assert.match(devicePanel, /data-testid="workspace-device-arrangement-interaction-layer"/, '独立设备排布编辑器缺少真实交互层')
  assert.match(devicePanel, /data-testid="workspace-device-arrangement-selected-outline"/, '独立设备排布编辑器缺少选中高亮边框')
  assert.match(devicePanel, /data-testid="workspace-device-arrangement-layer-actions"/, '独立设备排布编辑器缺少图层操作')
  ...
  assert.match(devicePanel, /function beginItemInteraction\(/, '独立设备排布编辑器缺少画布设备拖拽交互')
  assert.match(devicePanel, /mode: 'move' \| 'resize' \| 'rotate'/, '独立设备排布编辑器缺少移动、缩放、旋转交互状态')
  assert.match(devicePanel, /function duplicateItem\(/, '独立设备排布编辑器缺少设备复制动作')
  assert.match(devicePanel, /function moveItemLayer\(/, '独立设备排布编辑器缺少图层顺序调整动作')

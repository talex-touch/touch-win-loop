...
it('资源管理器把设备排布作为独立资源创建，不再走设计画布初始 scene', async () => {
  ...
  assert.match(projectPage, /await openProjectResourcePreview\(createdResourceId, \{ forceReload: true \}\)/, '项目页创建设备排布后未直接打开新资源')
...
it('独立设备排布编辑器提供模板、手动 transform、批量导出与截图资产化', async () => {
  const [devicePanel, helper, projectPage] = await Promise.all([

...
it('答辩左栏承载评委席、人设编辑与评分卡，保持线性 aside 结构', async () => {
  ...
  assert.match(source, /activePersonaPopover\.scoringRubric\.map\(item => item\.name\)\.join\('、'\)/, 'persona popover 未展示评分维度明细')
...
it('答辩主区切成无 tabs 的比赛状态驾驶舱，并提供主动作入口', async () => {
  ...
  assert.match(source, /\.workspace-defense-workbench__corner-preview \{[\s\S]*position: fixed;[\s\S]*right: max\(24px, env\(safe-area-inset-right\)\);[\s\S]*bottom: calc\(104px \+ env\(safe-area-inset-bottom\)\);[\s\S]*z-index: 11;/, '本地视频预览未固定到答辩页面右下角')

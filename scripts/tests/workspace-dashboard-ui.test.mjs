...
it('项目工作区头像菜单已迁移到左下角 rail，头部改为 AI 按钮入口', async () => {
  ...
  assert.match(userRailMenuSource, /z-index: 120;/, '左下角头像菜单浮层层级不足，仍可能被工作区内容遮挡')
  ...
  assert.match(workspaceSource, /:ai-collapsed="rightSidebarCollapsed"/, '项目工作区未向 WorkspaceHeader 透传右栏收起态')
...
it('项目主面板在无标签页时隐藏上方 chrome，并改成全幅默认仪表盘空态', async () => {
  const [source, chromeSource, tabStripSource, tabContextMenuSource, emptyStateSource, workspaceSource] = await Promise.all([
    readFile(WORKSPACE_MAIN_PANEL_FILE, 'utf8'),
  ...
  assert.match(tabContextMenuSource, /class="workspace-tab-context-menu__item workspace-tab-context-menu__item--danger"[\s\S]*@click="emit\('closeAll'\)"/, '标签页右键菜单缺少关闭所有动作')
  assert.match(tabContextMenuSource, /workspace-tab-context-menu__divider/, '标签页右键菜单缺少分割线')
  assert.match(tabContextMenuSource, /workspace-tab-context-menu__icon/, '标签页右键菜单缺少图标')
  assert.match(tabContextMenuSource, /width: 176px;/, '标签页右键菜单宽度未收窄')
  ...
  assert.match(emptyStateSource, /workspace-main-empty-state__watermark" aria-hidden="true">[\s\S]*<span>WIN<\/span>[\s\S]*<span>LOOP<\/span>/, '无标签页空态缺少换行的 WIN \/ LOOP 水印')
  assert.doesNotMatch(emptyStateSource, /当前没有打开的标签页/, '无标签页空态仍保留多余说明文案')
  assert.match(emptyStateSource, /border-radius: 12px;/, '无标签页空态按钮圆角仍过于圆润')
  assert.doesNotMatch(emptyStateSource, /transform: translateY\(-1px\);/, '无标签页空态按钮 hover 仍保留位移动效')
  assert.match(emptyStateSource, /\.workspace-main-empty-state__button:hover \{\s+background: #2563eb;\s+border-color: #2563eb;\s+color: #ffffff;/, '无标签页空态按钮 hover 未切为主按钮态')
  assert.match(emptyStateSource, /workspace-main-empty-state__button[\s\S]*打开默认仪表盘/, '无标签页空态缺少中央默认仪表盘按钮')
...
it('项目仪表盘移除本地 mock 分析数据，并接入准备态与真实规则加载链路', async () => {
  const [workspaceSource, panelSource, dashboardTabSource, statusBarSource] = await Promise.all([
    readFile(PROJECT_WORKSPACE_FILE, 'utf8'),
    readFile(WORKSPACE_MAIN_PANEL_FILE, 'utf8'),

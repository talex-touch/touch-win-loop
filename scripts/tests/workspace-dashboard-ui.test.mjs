...
it('项目页按真实 AI 配置态禁用功能，并将底部运行状态收口到右下角', async () => {
  ...
  assert.match(workspaceSource, /function buildDocumentAssistUserPrompt\(action: AiWorkspaceDocumentAction, extraInstruction = ''\): string \{/, '项目页未允许文稿助手拼接额外输入要求')
  assert.match(workspaceSource, /if \(!ensureAiFeatureAvailable\(resolveDocumentAssistFeatureKey\(payload\.action\)\)\)\s+return/, '文档 AI 入口未按动作前置拦截未配置态')
  ...
  assert.match(workspaceSource, /if \(aiMode\.value === 'document_assist'\) \{[\s\S]*await submitDocumentAssistFromComposer\(\)[\s\S]*return[\s\S]*\}/, '项目页未将文稿助手输入框发送接回 document_assist 链路')

...
it('team 创建弹窗复用基础设置编辑器，并支持仅创建与复选框竞赛列表', async () => {
  ...
  assert.match(teamSource, /const createForm = reactive<WorkspaceProjectCommonForm & \{ contestIds: string\[\] \}>\(\{[\s\S]*\.\.\.createEmptyProjectCommonForm\(\)/, 'Team 页创建表单未复用 WorkspaceProjectCommonForm 基础结构')

...
it('平台首页改成纯 Loopy 问答页，侧边导航同步切换品牌', async () => {
  ...
  assert.match(dashboardSource, /loopy-page-composer__tools/, 'Loopy 首页缺少输入区快捷动作栏')
  assert.match(dashboardSource, /loopy-page-ai-selector/, 'Loopy 首页未将 AI 选择器收口到输入区右侧')
  assert.match(dashboardSource, /AI 回答速度/, 'Loopy 首页缺少回答速度提示')
  assert.match(dashboardSource, /附件[\s\S]*@ 资料[\s\S]*\/ 命令/, 'Loopy 首页缺少底部快捷动作入口')
  assert.match(dashboardSource, /buildDialogTitlePreview/, 'Loopy 首页未提供首条用户消息的即时标题预览')
  assert.match(dashboardSource, /class="[^"]*flex[^"]*h-full[^"]*min-h-0[^"]*(?:min-w-0[^"]*w-full|w-full[^"]*min-w-0)/, 'Loopy 首页根节点未占满 dashboard shell 可用宽高')
  assert.match(dashboardSource, /data-testid="dashboard-loopy-session-list"[\s\S]*class="[^"]*p-2[^"]*flex-1[^"]*min-h-0[^"]*overflow-y-auto/, 'Loopy 首页左栏未改成更紧凑内部滚动')
  assert.match(dashboardSource, /data-testid="dashboard-loopy-messages"[\s\S]*class="[^"]*loopy-page-scroll[^"]*flex-1[^"]*min-h-0[^"]*overflow-y-auto/, 'Loopy 首页右侧消息区未改成更紧凑内部滚动')
  ...
  assert.doesNotMatch(dashboardSource, /loopy-page-speed/, 'Loopy 首页仍保留独立底部 AI 速度条')
  assert.doesNotMatch(dashboardSource, /loopy-page-welcome/, 'Loopy 首页仍保留欢迎横幅')
...
it('项目基础设置编辑器被抽成公共组件，并沿用 icon 与 accentColor 保存链路', async () => {
  ...
  assert.match(editorSource, /class="project-basic-settings-editor__hero"/, '基础设置公共组件缺少稳定的头部双栏容器')
  assert.match(editorSource, /@media \(min-width: 1024px\) \{[\s\S]*\.project-basic-settings-editor__hero \{[\s\S]*grid-template-columns: minmax\(0, 1\.06fr\) minmax\(320px, 0\.94fr\)/, '基础设置公共组件未在桌面端切换为左右双栏')
  assert.match(editorSource, /gap-4 grid grid-cols-1 lg:grid-cols-2/, '基础设置公共组件未在桌面端将图标与强调色改成双栏')
  assert.match(projectSettingsTabSource, /type WorkspaceProjectSettingsSectionId =/, '工作区项目设置面板缺少 section 级别的单面板类型定义')
  assert.match(projectSettingsTabSource, /activeSettingsSectionId\?: WorkspaceProjectSettingsSectionId \| ''/, '工作区项目设置面板缺少当前 section 入参')
  assert.match(projectSettingsTabSource, /v-show="shouldShowSection\('projectOverview'\)"/, '工作区项目设置面板未按左侧 navbar 选择只显示当前 section')
...
it('项目工作区头像菜单已迁移到左下角 rail，头部改为 AI 按钮入口', async () => {
  ...
  assert.match(panelSource, /watch\(\(\) => props\.openDisplayPreferencesSignal, \(next, previous\) => \{[\s\S]*ensureFixedTabOpen\('settings', true\)[\s\S]*activateWorkspaceSettingsSection\('displayPreferences', \{ behavior: 'auto' \}\)/, 'WorkspaceMainPanel 未把显示偏好 signal 定向到 Personal \/ 显示偏好 section')
...
it('项目仪表盘移除本地 mock 分析数据，并接入准备态与真实规则加载链路', async () => {
  ...
  assert.match(workspaceSource, /:mapping-loading="mappingFirstLoadLoading"/, '项目页未向主面板透传规则首屏加载态')
  ...
  assert.match(workspaceSource, /const mappingLoadedScopeKey = ref\(''\)/, '项目页缺少仪表盘映射作用域加载快照')
  assert.match(workspaceSource, /const mappingFirstLoadLoading = computed\(\(\) => \{[\s\S]*mappingLoadedScopeKey\.value !== mappingScopeKey\.value/, '项目页未拆分仪表盘首屏加载态')
  assert.match(workspaceSource, /const mappingRefreshing = computed\(\(\) => \{[\s\S]*mappingLoadedScopeKey\.value === mappingScopeKey\.value/, '项目页未拆分仪表盘后台刷新态')
  assert.match(workspaceSource, /:mapping-loading="mappingFirstLoadLoading"/, '项目页未向主面板透传仪表盘首屏加载态')
  assert.match(workspaceSource, /:mapping-refreshing="mappingRefreshing"/, '项目页未向主面板透传仪表盘后台刷新态')
...
it('loopy 数据工作台通过共享知识索引状态驱动主 Tab 与状态栏入口', async () => {
  const [workspaceSource, statusBarSource, panelSource, shellSource, knowledgeComposableSource, loopyDataTabSource] = await Promise.all([
    readFile(PROJECT_WORKSPACE_FILE, 'utf8'),
    readFile(WORKSPACE_STATUS_BAR_FILE, 'utf8'),
    readFile(WORKSPACE_MAIN_PANEL_FILE, 'utf8'),
    readFile(WORKSPACE_SHELL_COMPOSABLE_FILE, 'utf8'),
  ...
  assert.match(shellSource, /const openLoopyDataSignal = ref\(0\)/, '工作区 shell 未提供 Loopy 数据打开信号')
  assert.match(shellSource, /\['dashboard', 'meeting', 'members', 'flow', 'settings', 'loopy_data'\]/, '工作区主 Tab 合法集合未纳入 loopy_data')
  assert.match(knowledgeComposableSource, /export function useWorkspaceProjectKnowledge\(projectId: MaybeRefString\)/, '缺少项目级知识索引共享 composable')
  assert.match(knowledgeComposableSource, /endpoint\(`\/projects\/\$\{normalizedProjectId\}\/knowledge\/index-status`\)/, '共享 composable 未接入知识索引 dashboard 接口')
  assert.match(knowledgeComposableSource, /const shouldPoll = computed\(\(\) => Boolean\(resolvedProjectId\.value\) && hasActiveKnowledgeWork\(currentDashboard\.value\)\)/, '共享 composable 未按 backlog 轮询知识索引状态')
  assert.match(panelSource, /openLoopyDataSignal\?: number/, '主面板缺少 openLoopyDataSignal 入参')
  assert.match(panelSource, /id: 'loopy_data'/, '主面板未注册 Loopy 数据固定 Tab')
  assert.match(panelSource, /WorkspaceLoopyDataTab/, '主面板未挂载 Loopy 数据工作台组件')
  assert.match(panelSource, /@reload="emit\('reloadProjectKnowledge'\)"/, '主面板未透传 Loopy 数据刷新事件')
  assert.match(loopyDataTabSource, /WorkspaceLoopyDataHealthView/, 'Loopy 数据工作台缺少索引健康视图')
  assert.match(loopyDataTabSource, /WorkspaceLoopyDataRelationsView/, 'Loopy 数据工作台缺少关系探索视图')
  assert.match(loopyDataTabSource, /WorkspaceLoopyDataSemanticSpace/, 'Loopy 数据工作台缺少真实语义空间视图')
  assert.match(statusBarSource, /loopyDataProgressPercent\?: number/, '状态栏缺少 Loopy 数据进度入参')
  assert.match(statusBarSource, /openLoopyData: \[\]/, '状态栏缺少打开 Loopy 数据事件')
  assert.match(statusBarSource, /workspace-status-loopy__progress/, '状态栏缺少 Loopy 数据进度背景结构')
  assert.match(statusBarSource, /loopyDataSemanticLayoutUpdatedAt\?: string/, '状态栏缺少语义布局 freshness 入参')
  assert.match(statusBarSource, /Analytics/, '状态栏缺少 Loopy analytics tooltip')
  assert.match(statusBarSource, /Loopy 数据/, '状态栏缺少 Loopy 数据入口文案')
  assert.match(workspaceSource, /useWorkspaceProjectKnowledge\(activeProjectId\)/, '项目页未挂载共享知识索引 composable')
  assert.match(workspaceSource, /:open-loopy-data-signal="openLoopyDataSignal"/, '项目页未将 Loopy 数据打开信号透传给主面板')
  assert.match(workspaceSource, /:project-knowledge-dashboard="projectKnowledgeDashboard"/, '项目页未向主面板透传知识索引 dashboard')
  assert.match(workspaceSource, /:loopy-data-health-state="projectKnowledgeDiagnostics\.healthState"/, '项目页未向状态栏透传 Loopy 诊断状态')
  assert.match(workspaceSource, /:loopy-data-semantic-layout-updated-at="projectKnowledgeDashboard\?\.analytics\.semanticLayoutUpdatedAt \|\| ''"/, '项目页未向状态栏透传语义布局 freshness')
  assert.match(workspaceSource, /@open-loopy-data="openLoopyDataPanel"/, '项目页未接入状态栏打开 Loopy 数据事件')
})
...
it('项目页按真实 AI 配置态禁用功能，并将底部运行状态收口到右下角', async () => {
  ...
  assert.match(workspaceSource, /assistantMetadata = \{[\s\S]*result\.documentDraft[\s\S]*agentDocDraft: result\.documentDraft[\s\S]*result\.workflowDraft[\s\S]*workflowDraft: result\.workflowDraft[\s\S]*\}/, '项目页未把 AgentDoc \/ workflow 草案写入消息元数据')

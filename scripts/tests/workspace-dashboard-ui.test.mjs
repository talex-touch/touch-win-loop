...
const DASHBOARD_RIGHT_RAIL_FILE = resolve(process.cwd(), 'app/components/dashboard/DashboardRightRail.vue')
const DASHBOARD_COMPETITION_FEED_FILE = resolve(process.cwd(), 'app/components/dashboard/DashboardCompetitionFeed.vue')
...
const TEAM_PROJECT_DETAIL_DIALOG_FILE = resolve(process.cwd(), 'app/components/team/TeamProjectDetailDialog.vue')
...
const PROJECT_IDENTITY_THEME_CARD_FILE = resolve(process.cwd(), 'app/components/project/ProjectIdentityThemeCard.vue')
const PROJECT_DISPLAY_PREVIEW_CARD_FILE = resolve(process.cwd(), 'app/components/project/ProjectDisplayPreviewCard.vue')
...
it('项目卡展示图标徽标、底部摘要区、快捷操作区与稳定 display fallback', async () => {
  ...
  const detailDialogSource = await readFile(TEAM_PROJECT_DETAIL_DIALOG_FILE, 'utf8')
  ...
  assert.match(detailDialogSource, /<ProjectDisplayPreviewCard/, '项目信息弹窗未复用统一项目预览卡片')
...
it('平台首页改成纯 Loopy 问答页，侧边导航同步切换品牌', async () => {
  ...
  assert.match(dashboardSource, /mode:\s*'loopy_page'/, 'Loopy 首页未使用独立 loopy_page 会话模式')
  assert.match(dashboardSource, /loopy-page-composer-panel/, 'Loopy 首页底部工具缺少真实弹层')
  assert.match(dashboardSource, /handleQuickAction\(action\.id\)/, 'Loopy 首页底部工具仍不是可执行动作')
  assert.match(dashboardSource, /attachmentInputRef/, 'Loopy 首页附件按钮未绑定文件选择')
  assert.match(dashboardSource, /平台资料[\s\S]*项目资料/, 'Loopy 首页资料选择缺少平台/项目双来源')
  assert.match(dashboardSource, /\/resources\?\$\{query\.toString\(\)\}/, 'Loopy 首页平台资料未接入搜索接口')
  assert.match(dashboardSource, /\/projects\/\$\{encodeURIComponent\(project\.id\)\}\/resources/, 'Loopy 首页项目资料未接入项目资料接口')
  assert.match(dashboardSource, /\/projects\?teamId=\$\{encodeURIComponent\(normalizedWorkspaceId\)\}/, 'Loopy 首页项目选择未接入工作空间项目列表')
  assert.match(dashboardSource, /projectId:\s*''[\s\S]*projectTitle:\s*project\?\.title/, 'Loopy 首页项目上下文不应参与会话 scope')
  ...
  assert.doesNotMatch(dashboardSource, /loopy-page-speed(?:\s|")/, 'Loopy 首页仍保留独立底部 AI 速度条')
  ...
  assert.match(loopyDialogSource, /mode:\s*chatMode\.value/, 'Loopy 对话 composable 未按调用方模式隔离会话')
...
it('原 dashboard 业务模块已整页迁入 Team 项目台', async () => {
  const teamSource = await readFile(WORKSPACE_DETAIL_FILE, 'utf8')
  const rightRailSource = await readFile(DASHBOARD_RIGHT_RAIL_FILE, 'utf8')
  const competitionFeedSource = await readFile(DASHBOARD_COMPETITION_FEED_FILE, 'utf8')
  ...
  assert.doesNotMatch(teamSource, /<DashboardPlatformPanel/, 'Team 项目台仍在渲染平台能力中心')
  assert.match(teamSource, /<DashboardInsights/, 'Team 项目台未接入 AI 洞察')
  assert.match(rightRailSource, /<DashboardCompetitionFeed/, 'Team 项目台右栏未接入赛事动态流')
  assert.match(rightRailSource, /scrollable/, '赛事动态流未配置右栏内部滚动')
  assert.match(competitionFeedSource, /scrollable\?: boolean/, '赛事动态流组件缺少内部滚动开关')
  assert.match(competitionFeedSource, /overflow-y-auto/, '赛事动态流组件未把列表区改为内部滚动')
  assert.match(competitionFeedSource, /v-else-if="competitions\.length === 0"[\s\S]*overflow-y-auto/, '赛事动态流空态未纳入右栏内部滚动区域')
  assert.match(teamSource, /<DashboardRightRail/, 'Team 项目台未接入右侧工作台区块')
  assert.doesNotMatch(teamSource, /portalCards/, 'Team 项目台仍保留平台能力中心 portal cards')
  assert.doesNotMatch(teamSource, /teamQuickActions|:quick-actions=/, 'Team 项目台仍保留工作台快捷访问数据链路')
  assert.doesNotMatch(teamSource, /:loading="loading"|workspace-project-skeleton/, 'Team 项目台标题壳仍在渲染全宽 loading 骨架')
  assert.match(teamSource, /:skill-metrics="skillMetrics"/, 'Team 项目台未透传个人竞争力分析数据')
  assert.match(teamSource, /const workspaceProjects = computed\(\(\) => \{[\s\S]*resolveProjectTeamId\(item\) === workspaceId/, 'Team 项目台未按当前 Team 收敛项目范围')
  assert.match(teamSource, /const boundContestIds = computed\(\(\) => \{[\s\S]*resolveProjectBoundContestIds\(project\)/, 'Team 项目台未从项目绑定关系派生赛事集合')
  assert.match(teamSource, /const PLACEHOLDER_CONTEST_IDS = new Set\(\['quick_draft'\]\)/, 'Team 项目台未排除 quick_draft 占位赛事')
  assert.match(teamSource, /for \(const contestId of project\.contestIds \|\| \[\]\)[\s\S]*append\(project\.contestId\)/, 'Team 项目台未同时兼容 contestIds 和旧 contestId')
  assert.match(teamSource, /const boundContestDetailMap = ref<Record<string, Contest>>\(\{\}\)/, 'Team 项目台缺少绑定赛事详情兜底缓存')
  assert.match(teamSource, /async function loadBoundContestDetails\(workspaceId: string\) \{[\s\S]*\/contests\/\$\{encodeURIComponent\(contestId\)\}/, 'Team 项目台未按绑定赛事 ID 拉取真实详情兜底')
  assert.match(teamSource, /mergeContestDetailPayload\(result\.value\.data\)/, 'Team 项目台未把赛事详情中的时间节点与资料合并回右栏数据源')
  assert.match(teamSource, /const teamScheduleItems = computed<DashboardScheduleItem\[\]>\(\(\) => \{[\s\S]*createTimelineScheduleCandidate\(contest, timeline\)/, 'Team 项目台本周日程未使用绑定赛事真实时间节点')
  assert.match(teamSource, /createContestDeadlineScheduleCandidate\(contest\)/, 'Team 项目台本周日程缺少提交截止兜底')
  assert.match(teamSource, /const teamCompetitions = computed<DashboardCompetition\[\]>\(\(\) => \{[\s\S]*resolveContestPrimaryTimeline\(contest\)/, 'Team 项目台赛事动态未从绑定赛事真实时间节点派生')
  assert.match(teamSource, /const teamCompetitions = computed<DashboardCompetition\[\]>\(\(\) => \{[\s\S]*resolveLatestPolicyNotice\(contest\)/, 'Team 项目台赛事动态未从绑定赛事真实公告派生')
  assert.match(teamSource, /\.filter\(item => item\.category === 'policy_notice'\)/, 'Team 项目台未用真实政策通知资料生成公告动态')
  assert.match(teamSource, /:competitions="visibleTeamCompetitions"/, 'Team 项目台右栏仍未使用绑定赛事动态')
  assert.match(teamSource, /:schedule-items="teamScheduleItems"/, 'Team 项目台右栏仍未使用绑定赛事日程')
  assert.doesNotMatch(teamSource, /visibleCompetitions/, 'Team 项目台仍在消费全局赛事动态')
  assert.doesNotMatch(teamSource, /scheduleItems,/, 'Team 项目台仍在消费全局日程数据')
  assert.match(teamSource, /<DashboardInsights[\s\S]*<TeamProjectOverview[\s\S]*<DashboardRightRail/, 'Team 项目台未把项目列表放到左栏 AI 洞察下方')
  assert.match(teamSource, /class="team-dashboard-page[^"]*h-full[^"]*overflow-hidden/, 'Team 项目台外层仍可能触发整页滚动')
})

it('侧边导航项目台入口直达当前 active workspace', async () => {
  const sidebarSource = await readFile(DASHBOARD_SIDEBAR_FILE, 'utf8')

  assert.match(sidebarSource, /function resolveMenuItemTo\(item: DashboardMenuItem\): string \{[\s\S]*item\.id === 'team' && selectedWorkspaceId\.value[\s\S]*`\/team\/\$\{selectedWorkspaceId\.value\}`/, '侧边导航项目台入口未按当前 workspace 生成直达路径')
  assert.match(sidebarSource, /:to="resolveMenuItemTo\(item\)"/, '侧边导航主菜单仍使用静态 item.to')
  assert.match(sidebarSource, /\^\\\/team\\\/\[\^\/\]\+\(\?:\\\/project\\\/\[\^\/\]\+\)\?\$/, '侧边导航未识别 Team 项目台及项目工作区 active 态')
})
...
it('team 项目台走 dashboard shell，只有项目工作区保持全屏', async () => {
  ...
  assert.match(layoutSource, /const isTeamDashboardDetail = computed\(\(\) => \{[\s\S]*\^\\\/team\\\/\[\^\/\]\+\$[\s\S]*\}\)/, 'dashboard layout 未识别 Team 项目台详情页')
  assert.match(layoutSource, /:class="isTeamDashboardDetail \? 'overflow-hidden' : 'overflow-y-auto'"/, 'Team 项目台详情页仍使用 dashboard shell 全页滚动')
...
it('项目基础设置编辑器被抽成公共组件，并沿用 icon 与 accentColor 保存链路', async () => {
  ...
  const identityThemeCardSource = await readFile(PROJECT_IDENTITY_THEME_CARD_FILE, 'utf8')
  const displayPreviewCardSource = await readFile(PROJECT_DISPLAY_PREVIEW_CARD_FILE, 'utf8')
  const projectDisplaySource = await readFile(resolve(process.cwd(), 'shared/constants/project-display.ts'), 'utf8')
  ...
  assert.match(editorSource, /<ProjectIdentityThemeCard/, '基础设置公共组件未复用统一项目标识主题卡片')
  ...
  assert.match(identityThemeCardSource, /项目标识与主题/, '统一项目标识主题卡片缺少标题')
  assert.match(identityThemeCardSource, /data-testid="project-identity-theme-card"/, '统一项目标识主题卡片缺少稳定测试标识')
  assert.match(identityThemeCardSource, /data-testid="project-identity-theme-trigger"/, '统一项目标识主题卡片缺少图标触发器')
  assert.match(identityThemeCardSource, /data-testid="project-identity-theme-popover"/, '统一项目标识主题卡片缺少图标主题编辑 popover')
  assert.match(identityThemeCardSource, /trigger="click"/, '统一项目标识主题卡片未使用点击触发编辑 popover')
  assert.match(identityThemeCardSource, /<ProjectDisplayPreviewCard/, '统一项目标识主题卡片未复用项目预览卡片')
  assert.match(identityThemeCardSource, /type="color"/, '统一项目标识主题卡片缺少自定义颜色取色器')
  assert.match(identityThemeCardSource, /usingCustomAccent/, '统一项目标识主题卡片缺少自定义颜色状态')
  assert.match(identityThemeCardSource, /resolveIconKind\(option\.value\) === 'text'/, '统一项目标识主题卡片缺少文字图标样式')
  assert.match(identityThemeCardSource, /project-identity-theme-card__option-solid-icon/, '统一项目标识主题卡片缺少纯色图标样式')
  assert.match(identityThemeCardSource, /:data-tooltip="option\.label"/, '统一项目标识主题卡片缺少主题色名称 tooltip')
  assert.match(identityThemeCardSource, /project-identity-theme-card__icon-scroll/, '统一项目标识主题卡片缺少图标区内部滚动容器')
  assert.match(identityThemeCardSource, /max-height: 210px/, '统一项目标识主题卡片缺少压缩后的图标滚动高度')
  assert.match(identityThemeCardSource, /grid-template-columns: repeat\(5, minmax\(0, 1fr\)\)/, '统一项目标识主题卡片缺少压缩后的图标网格列数')
  assert.doesNotMatch(identityThemeCardSource, /<p class="project-identity-theme-card__label">\s+项目简介\s+<\/p>/, '统一项目标识主题卡片仍常驻展示项目简介分栏')
  assert.match(displayPreviewCardSource, /data-testid="project-display-preview-card"/, '项目预览卡片缺少稳定测试标识')
  assert.match(displayPreviewCardSource, /project-display-preview-card__monogram/, '项目预览卡片缺少项目简称徽标')
  assert.match(displayPreviewCardSource, /iconKind === 'text'/, '项目预览卡片缺少文字图标渲染')
  assert.match(displayPreviewCardSource, /iconKind === 'symbol'/, '项目预览卡片缺少系统图标渲染')
  assert.match(displayPreviewCardSource, /project-display-preview-card__icon--solid/, '项目预览卡片缺少纯色图标渲染')
  assert.match(projectDisplaySource, /auto_awesome/, '项目图标库缺少扩展图标')
  assert.match(projectDisplaySource, /workspace_premium/, '项目图标库缺少奖项类扩展图标')
  assert.match(projectDisplaySource, /PROJECT_DISPLAY_SYMBOL_ICON_OPTIONS\[hash % PROJECT_DISPLAY_SYMBOL_ICON_OPTIONS\.length\]/, '默认项目图标不应从文字或纯色模式里随机生成')
  assert.match(editorSource, /data-testid="project-basic-settings-editor"/, '基础设置公共组件缺少稳定测试标识')
  assert.match(editorSource, /class="project-basic-settings-editor__fields"/, '基础设置公共组件缺少标题摘要字段容器')
  assert.doesNotMatch(editorSource, /project-basic-settings-editor__hero/, '基础设置公共组件仍保留旧的头部双栏容器')
  assert.doesNotMatch(editorSource, /basis-\[148px\]/, '基础设置公共组件仍保留旧的大按钮网格')
...
it('team 创建弹窗复用基础设置编辑器，并支持两步创建与可选竞赛关联', async () => {
  const teamSource = await readFile(WORKSPACE_DETAIL_FILE, 'utf8')
  ...
  assert.doesNotMatch(teamSource, /topicBoardSeed: ProjectTopicBoardCreateSeed/, 'Team 页创建表单仍把选题板预配置混入新建流程')
  assert.doesNotMatch(teamSource, /TOPIC_BOARD_CREATE_SEED_STORAGE_PREFIX/, 'Team 页仍在创建后写入选题板自动生成 seed')
  ...
  assert.match(dialogSource, /type CreateProjectStep = 'basic' \| 'contest'/, 'Team 创建弹窗缺少两步状态模型')
  assert.match(dialogSource, /data-testid="team-create-project-stepper"/, 'Team 创建弹窗缺少步骤导航')
  assert.match(dialogSource, /data-testid="team-create-project-step-basic-panel"/, 'Team 创建弹窗缺少基本信息步骤面板')
  assert.match(dialogSource, /data-testid="team-create-project-step-contest-panel"/, 'Team 创建弹窗缺少关联竞赛步骤面板')
  assert.match(dialogSource, /data-testid="team-create-project-next-step-button"/, 'Team 创建弹窗缺少下一步按钮')
  assert.match(dialogSource, /2\. 关联竞赛（可选）/, 'Team 创建弹窗未标记竞赛关联为可选步骤')
  assert.match(dialogSource, /暂不关联竞赛/, 'Team 创建弹窗缺少不关联竞赛选项')
  assert.match(dialogSource, /关联竞赛/, 'Team 创建弹窗缺少关联竞赛选项')
  assert.match(dialogSource, /setContestLinkEnabled\(false\)/, 'Team 创建弹窗未支持显式跳过竞赛关联')
  assert.match(dialogSource, /emit\('update:contestIds', \[\]\)/, 'Team 创建弹窗跳过竞赛时未清空已选竞赛')
  assert.doesNotMatch(dialogSource, /创建后自动生成 AI 智能选题板/, 'Team 创建弹窗仍保留新建阶段选题板配置')
  assert.doesNotMatch(dialogSource, /ProjectTopicBoardCreateSeed/, 'Team 创建弹窗仍依赖选题板 seed 类型')
  assert.match(dialogSource, /const contestSearchQuery = ref\(''\)/, 'Team 创建弹窗缺少竞赛搜索关键词状态')
  assert.match(dialogSource, /const filteredContests = computed\(\(\) => \{/, 'Team 创建弹窗缺少竞赛过滤结果')
  assert.match(dialogSource, /data-testid="team-create-project-contest-search-input"/, 'Team 创建弹窗缺少竞赛搜索输入框')
  assert.match(dialogSource, /没有匹配的竞赛，可调整关键词或清空搜索。/, 'Team 创建弹窗缺少竞赛搜索空态')

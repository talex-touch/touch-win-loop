...
it('平台首页改成纯 Loopy 问答页，侧边导航同步切换品牌', async () => {
  ...
  assert.match(dashboardSource, /class="[^"]*mx-auto[^"]*flex[^"]*h-full[^"]*max-w-7xl[^"]*min-h-0/, 'Loopy 首页根节点未占满 dashboard shell 可用高度')
  assert.match(dashboardSource, /data-testid="dashboard-loopy-session-list"[\s\S]*class="[^"]*p-4[^"]*flex-1[^"]*min-h-0[^"]*overflow-y-auto/, 'Loopy 首页左栏未改成内部滚动')
  assert.match(dashboardSource, /data-testid="dashboard-loopy-messages"[\s\S]*class="[^"]*px-6[^"]*py-6[^"]*flex-1[^"]*min-h-0[^"]*overflow-y-auto/, 'Loopy 首页右侧消息区未改成内部滚动')

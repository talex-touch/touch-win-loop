...
it('无边画板已升级为真实引擎组件并移除 JSON 文本输入', async () => {
  const source = await readFile(WORKSPACE_MAIN_PANEL_FILE, 'utf8')
  assert.match(source, /<WorkspaceTldrawCanvas[\s\S]*@update:model-value="onCollabDrawModelUpdate"/, '白板未接入 WorkspaceTldrawCanvas 组件')
  assert.doesNotMatch(source, /请输入画布节点 JSON（数组）/, '白板仍暴露 JSON 文本输入入口')

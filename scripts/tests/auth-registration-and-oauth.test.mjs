...
it('共享个人设置弹框按个人信息与工作空间分组，并展示头像上传与空间改名入口', async () => {
  ...
  assert.match(source, /const currentUserId = computed/, '共享个人设置弹框未接入用户 ID 数据')
  assert.doesNotMatch(source, /账号绑定摘要/, '共享个人设置弹框不应再显示个人信息绑定摘要区块')
  assert.doesNotMatch(source, /管理页/, '共享个人设置弹框不应再展示平台管理员标签')
  assert.match(source, /编辑资料/, '共享个人设置弹框缺少资料编辑入口')
  assert.match(source, /logoutConfirmVisible/, '共享个人设置弹框未接入退出登录二次确认状态')
  assert.match(source, /confirmLogout/, '共享个人设置弹框未接入退出登录确认动作')

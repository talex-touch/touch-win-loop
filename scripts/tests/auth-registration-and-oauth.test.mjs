...
it('登录页提供第三方 OAuth 入口并根据注册开关切换文案', async () => {
  ...
  assert.match(pageSource, /使用飞书登录/, '登录页缺少平台飞书登录按钮')
  assert.match(pageSource, /feishuLoading \? '飞书登录中\.\.\.'/, '飞书内部无感登录尝试中应展示飞书登录中状态')
  assert.match(pageSource, /data-testid="login-feishu-button"/, '登录页缺少飞书登录测试标记')
  ...
  assert.match(composableSource, /manualFeishuLogin/, '登录页未暴露平台飞书登录动作')
  assert.match(composableSource, /\/auth\/feishu\/authorize/, '登录页未跳转飞书 authorize 接口')
  assert.match(composableSource, /tryFeishuAutoLogin/, '登录页缺少飞书内部自动登录尝试')
  assert.match(composableSource, /requestAuthCodeBySdk\(meta\.feishu\.appId\)/, '飞书内部无感登录未通过 SDK 获取 auth code')
  assert.match(composableSource, /\/auth\/feishu\/websdk-login/, '飞书内部无感登录未调用 WebSDK 登录接口')
  assert.match(composableSource, /resolveFeishuLoginTarget/, '飞书登录成功后未按用户权限决定默认落点')
  assert.doesNotMatch(composableSource, /飞书自动登录未完成，请改用账号密码登录。/, '飞书自动登录失败不应再打断账号密码登录')
})
...
it('登录页通过 auth layout 承载背景与 footer，并保留表单入口', async () => {
  ...
  assert.match(pageSource, /mx-auto max-w-\[680px\] min-w-\[520px\] w-full relative/, '登录页卡片未显式保持水平居中')

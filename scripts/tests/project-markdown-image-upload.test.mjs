...
const RICH_TEXT_IMAGE_EXTENSION_FILE = resolve(process.cwd(), 'app/components/editor/rich-text-editor-image-extension.ts')
...
it('编辑器 slash 菜单、图片接入与代码渲染能力已完整落地', async () => {
  ...
  const imageExtensionSource = await readFile(RICH_TEXT_IMAGE_EXTENSION_FILE, 'utf8')
  ...
  assert.match(editorSource, /const blockTypeMenuItems = computed\(\(\) => \{/, '编辑器未为标题下拉复用统一块级命令集')
  assert.match(editorSource, /data-testid="rich-text-editor-heading-menu"/, '编辑器未渲染标题下拉菜单')
  assert.match(editorSource, /data-testid="rich-text-editor-inline-search"/, '编辑器未渲染文档内联搜索容器')
  assert.match(editorSource, /openInlineSearch\(\): boolean \{/, '编辑器未暴露统一的内联搜索打开能力')
  ...
  assert.match(editorSource, /insertUploadingImagePlaceholder\(/, '编辑器未实现图片上传占位插入')
  assert.match(editorSource, /replaceUploadingImagePlaceholder\(/, '编辑器未实现图片上传占位原位替换')
  ...
  assert.match(commandsSource, /return normalized.length > 0 \? normalized : \[1, 2, 3, 4, 5, 6\]/, '统一命令注册表未将标题扩到 H1-H6')
  ...
  assert.match(imageExtensionSource, /createRichTextEditorImageExtension/, '编辑器未抽出图片 NodeView 扩展')
  assert.match(imageExtensionSource, /rich-text-editor__image-placeholder/, '图片 NodeView 未渲染上传中占位')
  assert.match(imageExtensionSource, /createResizeHandle\('left'\)/, '图片 NodeView 未渲染左侧 resize handle')
  assert.match(imageExtensionSource, /createResizeHandle\('right'\)/, '图片 NodeView 未渲染右侧 resize handle')
  assert.match(imageExtensionSource, /width: nextWidth/, '图片 NodeView 未在拖拽后写回宽度 attrs')
  ...
  assert.match(schemaSource, /export const CollabMarkdownImage = Image\.extend\(/, '共享 schema 未扩展 markdown image 节点')
  assert.match(schemaSource, /resourceId: \{\s+default: null,/, '共享 schema 未为图片节点保留 resourceId attr')
  assert.match(schemaSource, /width: \{\s+default: null,/, '共享 schema 未为图片节点保留 width attr')
  assert.match(schemaSource, /uploadStatus: \{\s+default: null,/, '共享 schema 未为图片节点保留 uploadStatus attr')
  assert.match(schemaSource, /uploadId: \{\s+default: null,/, '共享 schema 未为图片节点保留 uploadId attr')
})

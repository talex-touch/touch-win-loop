...
it('meeting asr gateway 已支持 openai-compatible 内置转写 provider', async () => {
  ...
  assert.match(source, /isDashScopeAsrProvider\(resolved\.provider,\s*ai\)/, '内置 ASR 未识别百炼 ASR Provider')
  assert.match(source, /transcribeDashScopeAsrAudio/, '百炼 ASR 未复用 DashScope ASR 适配器')
  assert.match(source, /modelRequired = !isCozeVoiceProvider\(resolved\.provider\) && !isDashScopeAsrProvider\(resolved\.provider,\s*ai\)/, 'Coze/百炼语音 ASR 仍被模型池强制校验拦截')
...
it('会议运行时与字幕帧接口已兼容内置 asr 会话失效场景', async () => {
  ...
  assert.match(runtimeSource, /modelRequired = channelRuntime\.provider\?\.capability !== 'voice'[\s\S]*!isDashScopeAsrProvider\(channelRuntime\.provider,\s*asrRuntime\)/, '会议运行时仍把 Coze/百炼语音 ASR 当作必须绑定模型')

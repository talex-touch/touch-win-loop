...
describe('project knowledge multimodal', () => {
  it('共享类型与运行时配置已扩展多模态字段', async () => {
    ...
    assert.match(channelsSource, /clientType: provider\.clientType/, 'provider registry 未向场景运行时透传 clientType')
  ...
  it('检索排序、引用展示与后台监控已具备多模态感知', async () => {
    ...
    assert.match(aiPromptsSource, /聊天接入类型/, 'AI 配置台未暴露 clientType 字段')
    ...
    assert.match(aiPromptsSource, /clientType: normalized\.clientType/, 'AI 配置保存未提交 clientType')
    assert.match(aiPromptsSource, /embeddingApiStyle: normalized\.embeddingApiStyle/, 'AI 配置保存未提交 embeddingApiStyle')
    assert.match(aiPromptsSource, /embeddingDimensions: Number\(normalized\.embeddingDimensions \|\| 1024\)/, 'AI 配置保存未提交 embeddingDimensions')
    assert.match(aiPromptsSource, /visionModel: normalized\.visionModel/, 'AI 配置保存未提交 visionModel')
  })
})

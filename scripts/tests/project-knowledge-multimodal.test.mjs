...
describe('project knowledge multimodal', () => {
  it('共享类型与运行时配置已扩展多模态字段', async () => {
    ...
    assert.doesNotMatch(providersGetSource, /visionModel: provider\.visionModel \|\| ''/, 'AI 配置读取接口不应再返回默认 visionModel')
    ...
    assert.doesNotMatch(providersPatchSource, /visionModel: provider\.visionModel \|\| ''/, 'AI 配置保存接口不应再返回默认 visionModel')
  ...
  it('worker 已生成图片投影、会议转写并升级索引版本', async () => {
    ...
    assert.match(visionSource, /resolveAiRuntimeForChannel\(runtime, 'knowledge_visual_projection'\)/, '视觉投影服务未改走独立知识视觉投影场景')
    assert.match(visionSource, /KNOWLEDGE_VISUAL_PROJECTION_RUNTIME_NOT_CONFIGURED/, '视觉投影未对缺配置做严格失败')
    assert.match(visionSource, /KNOWLEDGE_VISUAL_PROJECTION_IMAGE_TOO_LARGE/, '视觉投影未对超大图片做严格失败')
    assert.doesNotMatch(visionSource, /buildFallbackProjection/, '视觉投影不应再合成伪视觉摘要')
    assert.doesNotMatch(visionSource, /fallback:\s*\(\)\s*=>/, '视觉投影请求失败不应再返回 fallback 结果')
    assert.match(workerSource, /analyzeKnowledgeVisualProjection/, 'worker 未接入视觉投影分析')
    assert.doesNotMatch(workerSource, /visual\.fallbackUsed \? 'fallback_metadata' : 'vision_model'/, 'worker 不应把视觉投影失败伪装为 metadata fallback')
    ...
    assert.match(workerSource, /KNOWLEDGE_IMAGE_SOURCE_MISSING/, '图片投影缺源文件时未严格失败')
    assert.match(workerSource, /KNOWLEDGE_IMAGE_BUFFER_MISSING/, '图片投影缺文件缓冲时未严格失败')
    assert.doesNotMatch(workerSource, /projectionSource:\s*'fallback_metadata'/, 'worker 不应再生成 metadata 伪图片投影')
    assert.doesNotMatch(workerSource, /buildFallbackSummaryChunks/, 'worker 不应再用资源 metadata 生成 fallback summary 片段')
    assert.doesNotMatch(workerSource, /fallback_summary/, 'worker 不应再写入 fallback_summary 投影来源')
    assert.match(workerSource, /PROJECT_KNOWLEDGE_NO_EXTRACTABLE_CHUNKS/, '资源无真实可抽取片段时应严格失败')
    assert.doesNotMatch(workerSource, /document_visual_fallback/, 'worker 不应再生成文档视觉 fallback 图片片段')
    assert.doesNotMatch(workerSource, /isDocumentVisualFallbackCandidate/, 'worker 不应再保留文档视觉 fallback 候选分支')
  ...
  it('检索排序、引用展示与后台监控已具备多模态感知', async () => {
    ...
    assert.doesNotMatch(aiPromptsSource, /visionModel/, 'AI 配置台不应再维护默认 visionModel 字段')
    ...
    assert.match(aiPromptsSource, /knowledge_visual_projection/, 'AI 配置台未暴露视觉投影独立场景')
    assert.doesNotMatch(aiPromptsSource, /defaultsForm/, 'AI 配置台不应再维护 defaults 表单')
  })
})

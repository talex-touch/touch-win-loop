...
it('项目工作区首屏采用独立遮罩加载层，并保持主体内容先渲染', async () => {
  ...
  assert.match(source, /<Transition name="workspace-scene-loading-overlay">[\s\S]*<WorkspaceShellLoadingOverlay[\s\S]*v-if="workbenchSwitchLoading"[\s\S]*:progress="workbenchSwitchProgress"/, '项目页未将工作台切换遮罩接入 WinLoop 进度样式')
  assert.doesNotMatch(source, /visual="brand-lockup"|visual="text-progress"/, '项目页不应再为加载遮罩分叉视觉模式')
  ...
  assert.match(overlaySource, /const wordmarkBaseTextRef = ref<SVGTextElement \| null>\(null\)/, '独立加载遮罩缺少文本进度字样边界引用')
  assert.match(overlaySource, /const progressClipY = computed\(\(\) => \(wordmarkBounds\.value\.y \+ wordmarkBounds\.value\.height\) - progressClipHeight\.value\)/, '独立加载遮罩未为文本进度模式计算裁切区域')
  ...
  assert.match(overlaySource, /workspace-shell-loading-overlay__text-wordmark/, '独立加载遮罩缺少纯文本 WinLoop 进度字样容器')
  assert.match(overlaySource, /clipPath id="workspace-shell-loading-wordmark-fill"/, '独立加载遮罩缺少文本进度裁切路径')
  assert.doesNotMatch(overlaySource, /BrandLogo variant="lockup"[\s\S]*workspace-shell-loading-overlay__brand/, '独立加载遮罩不应再使用 lockup 图标品牌层')
  assert.doesNotMatch(overlaySource, /workspace-shell-loading-overlay__(wordmark|brand|fill)\b/, '独立加载遮罩不应再保留图标 lockup 进度层样式')
  assert.match(overlaySource, /workspace-shell-loading-overlay__percent/, '独立加载遮罩缺少百分比读数')
  assert.match(overlaySource, /workspace-shell-loading-overlay__text--fill/, '独立加载遮罩缺少纯文本进度填充层')
  assert.match(overlaySource, /clipPath id="workspace-shell-loading-wordmark-fill"/, '独立加载遮罩缺少纯文本进度的 SVG clipPath 实现')

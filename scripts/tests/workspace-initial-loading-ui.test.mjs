...
it('项目工作区首屏采用独立遮罩加载层，并保持主体内容先渲染', async () => {
  ...
  assert.match(source, /<Transition name="workspace-scene-loading-overlay">[\s\S]*<WorkspaceShellLoadingOverlay[\s\S]*v-if="workbenchSwitchLoading"[\s\S]*:progress="workbenchSwitchProgress"[\s\S]*visual="text-progress"/, '项目页未将工作台切换遮罩切回纯文本 WinLoop 进度样式')
  ...
  assert.match(overlaySource, /visual\?: 'brand-lockup' \| 'text-progress'/, '独立加载遮罩未暴露视觉模式开关')
  ...
  assert.match(overlaySource, /v-if="props\.visual === 'text-progress'"/, '独立加载遮罩未根据视觉模式切换纯文本进度样式')
  assert.match(overlaySource, /v-else class="workspace-shell-loading-overlay__wordmark"/, '独立加载遮罩未保留默认品牌 lockup 视觉分支')

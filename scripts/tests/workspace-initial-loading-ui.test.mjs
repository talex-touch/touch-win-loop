...
it('项目工作区首屏采用独立遮罩加载层，并保持主体内容先渲染', async () => {
  ...
  assert.match(source, /if \(workspaceShellLoading\.value\) \{\s+event\.preventDefault\(\)\s+event\.stopPropagation\(\)\s+closeWorkspaceContextMenu\(\{\s+restoreFocus: false,\s+\}\)\s+return\s+\}/, '项目页未在加载态拦截工作区右键菜单')
  assert.match(source, /watch\(\(\) => workspaceShellLoading\.value, \(loading\) => \{\s+if \(!loading \|\| !workspaceContextMenu\.visible\)\s+return\s+\s+closeWorkspaceContextMenu\(\{\s+restoreFocus: false,\s+\}\)\s+\}\)/, '项目页未在加载遮罩出现时关闭已打开的工作区菜单')

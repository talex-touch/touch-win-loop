...
it('答辩 realtime 领域类型、环境配置与 defense session state 扩展已落地', async () => {
  const [domainExportsSource, domainSource, nuxtConfigSource, envSource, storeSource, schemaSource, migrationSource, workspaceConfigSource, packageSource] = await Promise.all([
...
it('答辩 realtime 后端已拆成创建会话、bootstrap 和 events 三层编排', async () => {
  ...
  assert.match(bootstrapSource, /agentSelections/, 'bootstrap 接口未返回 Coze 评委智能体映射')
...
it('答辩 provider bridge、会议详情 sidecar 容器和透传链路已落地', async () => {
  ...
  assert.match(relaySource, /new UpstreamWebSocket\(runtime\.defenseRealtime\.qwen\.baseWsUrl, \{[\s\S]*Authorization: `Bearer \$\{qwenApiKey\}`/, '千问 relay 未通过服务端添加上游鉴权头')

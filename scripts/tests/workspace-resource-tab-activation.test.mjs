...
it('重新点击已打开资源标签时仅激活标签，不重复触发资源加载', async () => {
  ...
  assert.match(workspaceSource, /previewResourceId\.value = targetResourceId/, '资源标签激活时未同步当前预览资源 ID')
  ...
  assert.match(workspaceSource, /options\.forceReload !== true[\s\S]*previewResourceId\.value === targetResourceId[\s\S]*activeMainTabId\.value === targetTabId[\s\S]*return/, '资源预览打开链路缺少同资源复用守卫')
})

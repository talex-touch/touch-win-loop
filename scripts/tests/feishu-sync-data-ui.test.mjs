...
it('飞书已同步数据查询台会暴露共享类型与只读查询接口', async () => {
  ...
  assert.match(typeSource, /export interface FeishuSyncedDataMetrics \{[\s\S]*effectiveEntityTotal: number[\s\S]*latestRunSourceRowTotal: number[\s\S]*latestRunDuplicateExternalIdTotal: number[\s\S]*rawCountBasis: 'latest_run_per_sync_item'/, '共享类型未声明同步数据双口径指标')
  ...
  assert.match(typeSource, /export interface FeishuSyncedDataSyncItemOption \{[\s\S]*id: string[\s\S]*name: string[\s\S]*syncId: string[\s\S]*syncName: string[\s\S]*entityType: FeishuBitableSyncItemEntityType/, '共享类型未给同步项选项补齐 entityType')
  ...
  assert.match(typeSource, /export interface FeishuBitableSyncRunDiagnostics \{[\s\S]*processedUniqueExternalIdCount\?: number[\s\S]*sourceDuplicateExternalIdCount\?: number[\s\S]*sourceDuplicateExternalIdSamples\?: Array<\{[\s\S]*externalId: string[\s\S]*recordIds: string\[\]/, '共享类型未声明运行重复 externalId 诊断字段')
  ...
  assert.match(storeSource, /sourceDuplicateExternalIdCount[\s\S]*latest_run_duplicate_external_id_total/, '存储层未聚合最近一次运行重复 externalId 数')
  ...
  assert.doesNotMatch(apiSource, /listFeishuSyncedDataSyncItemOptions\(db,\s*\{\s*syncId:\s*input\.syncId\s*\}\)/, '查询接口不应再按当前查询 syncId 截断同步项元数据')
})
...
it('飞书已同步数据查询台页面会展示筛选项、结果表和详情抽屉', async () => {
  ...
  assert.match(pageSource, /业务去重\/覆盖[\s\S]*latestRunDuplicateExternalIdTotal/, '查询台未展示重复 externalId 折叠指标')
  assert.match(pageSource, /重复 externalId 会按业务实体合并覆盖/, '查询台未解释源行数与当前有效实体数差值')
  assert.match(pageSource, /已发布版本会保留/, '查询台未补充已发布版本保留但不重复计数的说明')
  assert.match(pageSource, /rawMetricNotice/, '查询台未展示原始导入口径说明')
  assert.match(pageSource, /const syncItemFilterOptions = computed\(\(\) => \{[\s\S]*item\.syncId === filters\.syncId/, '查询台未按当前 syncId 即时派生同步项选项')
  assert.match(pageSource, /const scopeOptions = computed\(\(\) => \{[\s\S]*item\.entityType[\s\S]*resultPayload\.value\.items/, '查询台未按同步项元数据与当前结果派生实体类型范围')
  assert.match(pageSource, /watch\(syncItemFilterOptions,\s*\(options\)\s*=>\s*\{[\s\S]*filters\.syncItemId = ''/, '查询台切换 syncId 后未清空失效的 syncItemId')
  assert.match(pageSource, /watch\(scopeOptions,\s*\(options\)\s*=>\s*\{[\s\S]*filters\.scope = ''/, '查询台切换 syncId 后未清空失效的 scope')
  assert.match(pageSource, /v-for="item in syncItemFilterOptions"/, '查询台同步项下拉未使用前端派生选项')
  assert.match(pageSource, /v-for="item in scopeOptions"/, '查询台实体类型下拉未使用前端派生范围')
})
...
it('运行样本分页接口、日志抽屉和单行模拟源字段分页已接入', async () => {
  ...
  assert.match(editorSource, /当前预检只会检查结构性缺失[\s\S]*业务去重\/覆盖[\s\S]*重复 externalId 样本排查/, '同步项执行区未明确预检仅覆盖结构性缺失')
  assert.match(editorSource, /新运行支持完整分页样本/, '日志抽屉未提示完整分页样本')
  assert.match(editorSource, /const itemId = currentItem\.value\.id[\s\S]*currentItemLogVisible\.value = true[\s\S]*loadCurrentItemLogDetail\(itemId, result\.runId\)/, '手动执行成功后未自动打开并选中本次执行日志')
  assert.match(editorSource, /syncRunDuplicateExternalIdText[\s\S]*业务去重\/覆盖/, '日志抽屉未展示重复 externalId 业务折叠诊断')
  assert.match(editorSource, /重复 externalId \/ 业务折叠样本/, '日志抽屉未展示重复 externalId 样本')

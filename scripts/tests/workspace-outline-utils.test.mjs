...
it('project outline adapter 会保留来源资源并生成行内引用元数据', async () => {
  const {
    buildProjectWorkspaceOutlineNodes,
  } = await loadWorkspaceOutlineUtils()

  const nodes = buildProjectWorkspaceOutlineNodes({
    projectOutline: [
      {
        id: 'outline-1',
        title: '会议模式',
        level: 0,
        order: 1,
        sourceResourceIds: ['resource-1', 'resource-2', 'resource-1', ''],
        confidence: 0.8,
        children: [],
      },
    ],
  ...
  assert.equal(nodes.length, 1)
  assert.equal(nodes[0]?.label, '1 会议模式')
  assert.equal(nodes[0]?.meta, '2 个来源')
  assert.deepEqual(nodes[0]?.sourceResourceIds, ['resource-1', 'resource-2'])
  assert.equal(nodes[0]?.locator.resourceId, 'resource-1')
})

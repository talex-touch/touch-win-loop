...
it('赛道同步映射会补齐扩展字段，并暴露赛道时间线实体选项', async () => {
  ...
  assert.match(configSource, /if \(entityType === 'track'\) \{[\s\S]*coverImageUrl:\s*''[\s\S]*location:\s*''[\s\S]*organizer:\s*''[\s\S]*undertaker:\s*''[\s\S]*participantRequirements:\s*''[\s\S]*teamRule:\s*''[\s\S]*awardRatio:\s*''[\s\S]*evidenceRequirements:\s*''[\s\S]*scoringPoints:\s*''[\s\S]*deductionItems:\s*''/m, '赛道默认模板未补齐扩展字段')
  assert.match(configSource, /if \(entityType === 'track_timeline'\) \{[\s\S]*trackExternalIdField:\s*''[\s\S]*year:\s*''[\s\S]*nodeType:\s*''[\s\S]*startAt:\s*''[\s\S]*endAt:\s*''[\s\S]*note:\s*''[\s\S]*sourceLink:\s*''/m, '赛道时间线默认模板未生成')
})
...
it('赛道同步执行会写入扩展字段并派生 rubric', async () => {
  ...
  assert.match(serviceSource, /patchAdminRubric\(db,\s*\{[\s\S]*scoringPoints,[\s\S]*deductionItems,[\s\S]*evidenceRequirements,/m, '赛道已有 rubric 时未走 patch')
  assert.match(serviceSource, /createAdminRubric\(db,\s*\{[\s\S]*trackId: input\.trackId,[\s\S]*dimensions:\s*\[\{[\s\S]*weight:\s*100/m, '赛道缺少 rubric 时未自动创建默认 rubric')
})
...
it('赛道时间线会走独立模型、独立 API 和独立同步链路', async () => {
  ...
  assert.match(domainSource, /export interface TrackTimeline \{[\s\S]*trackId: string[\s\S]*nodeType: TimelineNodeType/m, '领域模型缺少 TrackTimeline')

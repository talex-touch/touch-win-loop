...
describe('版本审批与赛道同步新流程', () => {
  it('赛道同步映射已覆盖赛道字段，并统一收敛到 timelineText', async () => {
    ...
    assert.match(componentSource, /currentSeason（当前届次）/, '赛道映射缺少当前届次字段')
    ...
    assert.match(configSource, /if \(entityType === 'track'\) \{[\s\S]*currentSeason:\s*''[\s\S]*timelineText:\s*''[\s\S]*deliverableTypes:\s*''/, '赛道默认模板未补齐当前届次、时间节点与提交内容')
    assert.match(typeSource, /export interface ContestReleaseTrackSnapshot \{[\s\S]*currentSeason\?: string/, '赛道 release snapshot 未声明 currentSeason')
    assert.match(serviceSource, /track:[\s\S]*'currentSeason'[\s\S]*'timelineText'/, '赛道预检字段未包含 currentSeason')
    assert.match(serviceSource, /async function applyTrackRecord[\s\S]*currentSeason,[\s\S]*input\.resolver\.getText\('currentSeason'\)[\s\S]*currentSeason,/, '赛道同步未写入 currentSeason 快照')
    ...
    assert.match(releaseStoreSource, /resolveContestReleaseEffectiveMetadata[\s\S]*snapshot\.tracks\.map\(item => item\.organizer\)[\s\S]*snapshot\.tracks\.map\(item => item\.participantRequirements\)[\s\S]*snapshot\.tracks\.map\(item => item\.currentSeason\)/, '发布校验未从赛道快照聚合主办方、参赛对象和届次')
    assert.match(releaseStoreSource, /inferLatestTimelineSeason\(\[\.\.\.snapshot\.timelines, \.\.\.snapshot\.trackTimelines\]\)/, '发布校验未用时间线年份兜底届次')
    assert.match(contestStoreSource, /resolveContestPublishEffectiveMetadata[\s\S]*input\.contest\.tracks[\s\S]*item\.organizer[\s\S]*item\.participantRequirements/, 'live 发布校验未从赛道聚合主办方和参赛对象')
  ...
  it('contest release snapshot 与发布链路已补齐赛事主字段', async () => {
    ...
    assert.match(releaseStoreSource, /patch: \{[\s\S]*organizer: effectiveMetadata\.organizer/, '发布版本时未按有效元信息回写 organizer')

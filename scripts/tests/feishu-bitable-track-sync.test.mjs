...
describe('版本审批与赛道同步新流程', () => {
  ...
  it('contest release snapshot 与发布链路不再依赖竞赛库已移除字段', async () => {
    ...
    assert.doesNotMatch(releaseStoreSource, /organizer: detail\.contest\.organizer \|\| ''/, 'release 基线快照不应带回 organizer')
    assert.doesNotMatch(releaseStoreSource, /coOrganizer: detail\.contest\.coOrganizer \|\| ''/, 'release 基线快照不应带回 coOrganizer')
    assert.doesNotMatch(releaseStoreSource, /participantRequirements: detail\.contest\.participantRequirements \|\| ''/, 'release 基线快照不应带回 participantRequirements')
    assert.doesNotMatch(releaseStoreSource, /teamRule: detail\.contest\.teamRule \|\| ''/, 'release 基线快照不应带回 teamRule')
    assert.doesNotMatch(releaseStoreSource, /currentSeason: detail\.contest\.currentSeason \|\| ''/, 'release 基线快照不应带回 currentSeason')

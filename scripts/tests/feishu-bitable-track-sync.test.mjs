...
describe('版本审批与赛道同步新流程', () => {
  ...
  it('contest release snapshot 与发布链路已补齐赛事主字段', async () => {
    const releaseStoreSource = await readSource('server/utils/release-store.ts')
    const serviceSource = await readSource('server/services/feishu/bitable-sync.ts')

    assert.match(serviceSource, /input\.resolver\.getText\('organizer'\)/, '竞赛同步未抓取 organizer')
    assert.match(serviceSource, /input\.resolver\.getText\('coOrganizer'\)/, '竞赛同步未抓取 coOrganizer')
    assert.match(serviceSource, /input\.resolver\.getText\('participantRequirements'\)/, '竞赛同步未抓取 participantRequirements')
    assert.match(serviceSource, /input\.resolver\.getText\('teamRule'\)/, '竞赛同步未抓取 teamRule')
    assert.match(serviceSource, /input\.resolver\.getText\('currentSeason'\)/, '竞赛同步未抓取 currentSeason')
    assert.match(releaseStoreSource, /organizer: detail\.contest\.organizer \|\| ''/, 'release 基线快照未带回 organizer')
    assert.match(releaseStoreSource, /participantRequirements: detail\.contest\.participantRequirements \|\| ''/, 'release 基线快照未带回 participantRequirements')
    assert.match(releaseStoreSource, /currentSeason: detail\.contest\.currentSeason \|\| ''/, 'release 基线快照未带回 currentSeason')
    assert.match(releaseStoreSource, /patch: \{[\s\S]*organizer: normalizeText\(snapshot\.contest\.organizer\)/, '发布版本时未回写 organizer')
    assert.match(releaseStoreSource, /patch: \{[\s\S]*participantRequirements: normalizeText\(snapshot\.contest\.participantRequirements\)/, '发布版本时未回写 participantRequirements')
    assert.match(releaseStoreSource, /patch: \{[\s\S]*currentSeason: normalizeText\(snapshot\.contest\.currentSeason\)/, '发布版本时未回写 currentSeason')
  })

...
it('飞书同步信息总览页会提供配置导出、公网分享、URL 和文件导入入口', async () => {
  const source = await readFile(OVERVIEW_FILE, 'utf8')

  assert.match(source, />\s*导入\s*</, '同步信息总览缺少导入 popover 入口')
  assert.match(source, /从配置 URL 导入/, '同步信息总览缺少配置 URL 导入入口')
  assert.match(source, /从配置文件导入/, '同步信息总览缺少配置文件导入入口')
  assert.match(source, /syncActionMenuVisibleId/, '同步信息行操作缺少更多 popover 状态')
  assert.match(source, />\s*更多\s*</, '同步信息行操作缺少更多 popover 入口')
  ...
  assert.match(source, /handleSyncConfigImportFileChange/, '页面缺少配置文件导入动作')
  assert.match(source, /syncConfigImportPackage/, '页面缺少配置文件导入包状态')

...
it('工作台 AI 正文改为消费 agent.streamEvents 真流式，并在可见输出后停止内部重试', async () => {
  ...
  assert.match(orchestratorSource, /export function extractWorkspaceStreamTextChunk\(chunk: unknown\): string \{/, '工作台 orchestrator 缺少模型 chunk 文本提取工具')
  assert.match(orchestratorSource, /export function extractWorkspaceLangGraphOutputMessages\(output: unknown\): Array<Record<string, unknown>> \{/, '工作台 orchestrator 缺少 LangGraph output messages 提取工具')
  assert.match(orchestratorSource, /export async function consumeWorkspaceAgentStream\(/, '工作台 orchestrator 缺少可复用的 stream 消费逻辑')
  assert.match(orchestratorSource, /if \(eventType === 'on_chat_model_stream'\) \{[\s\S]*extractWorkspaceStreamTextChunk\(event\.data\?\.chunk\)/, '工作台 orchestrator 未消费 on_chat_model_stream 文本 chunk')
  assert.match(orchestratorSource, /if \(eventType === 'on_chain_end' && toText\(event\.name\) === 'LangGraph'\) \{[\s\S]*extractWorkspaceLangGraphOutputMessages\(event\.data\?\.output\)/, '工作台 orchestrator 未从 LangGraph 收尾 output 提取最终 messages')

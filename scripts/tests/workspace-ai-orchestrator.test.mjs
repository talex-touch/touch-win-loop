...
it('文档增强会按 documentAction 映射到独立平台 AI 场景', async () => {
  ...
  assert.match(streamSource, /const channelKey = resolveWorkspaceChannelKey\(request\.mode \|\| 'dialog_ask', request\.context\?\.documentAction\)/, 'workspace 流接口未按 documentAction 解析场景 key')
})

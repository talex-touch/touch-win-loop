...
it('工作区显示偏好共享类型、存储表与后端优先级链已落地', async () => {
  ...
  assert.match(storeSource, /if \(input\.workspaceOverride\?\.fontSizePreset\)[\s\S]*source: 'workspace_override'/, '优先级未优先使用工作区个人覆盖')
  assert.match(storeSource, /if \(input\.userDefault\?\.fontSizePreset\)[\s\S]*source: 'user_default'/, '优先级未处理个人全局默认')
  assert.match(storeSource, /if \(input\.workspaceType === 'team' && input\.teamDefault\?\.fontSizePreset\)[\s\S]*source: 'team_default'/, '优先级未处理团队默认')
  assert.match(storeSource, /effective: getSystemWorkspaceDisplayPreferences\(\),[\s\S]*source: 'system_default'/, '优先级未回退到系统默认')
...
it('个人设置弹窗与项目工作区 Settings 已出现显示偏好入口', async () => {
  ...
  assert.match(dialogSource, /系统默认固定为 md/, '个人设置弹窗未展示系统默认说明')
...
it('项目工作区根节点已绑定生效字号，且样式仅在工作区作用域内覆盖', async () => {
  const [workspaceSource, appSource] = await Promise.all([

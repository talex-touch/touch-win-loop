...
describe('jenkins staging meeting stack', () => {
  it('compose template defines LiveKit and monitoring services behind the standard staging profile', async () => {
    ...
    assert.match(source, /\$\{MEETING_LIVEKIT_HTTP_PORT:-17880\}:7880\/tcp/, 'LiveKit HTTP 宿主端口未使用 staging 高位默认端口')
    assert.match(source, /\$\{MEETING_LIVEKIT_TCP_PORT:-17881\}:\$\{MEETING_LIVEKIT_TCP_PORT:-17881\}\/tcp/, 'LiveKit TCP 端口未使用 staging 高位默认端口')
    assert.match(source, /\$\{MEETING_LIVEKIT_RTC_UDP_RANGE:-51000-51100\}:\$\{MEETING_LIVEKIT_RTC_UDP_RANGE:-51000-51100\}\/udp/, 'LiveKit UDP range 未使用 staging 高位默认端口段')
  ...
  it('deploy script generates LiveKit and Prometheus configs during normal staging deployment', async () => {
    ...
    assert.match(source, /tcp_port: \$\{MEETING_LIVEKIT_TCP_PORT\}/, 'LiveKit TCP 配置未跟随宿主 staging TCP 端口')
    ...
    assert.match(source, /MEETING_LIVEKIT_HTTP_PORT="17880"/, 'staging 旧 LiveKit HTTP 默认端口未迁移')
    assert.match(source, /MEETING_LIVEKIT_TCP_PORT="17881"/, 'staging 旧 LiveKit TCP 默认端口未迁移')
    assert.match(source, /MEETING_LIVEKIT_RTC_UDP_RANGE="51000-51100"/, 'staging 旧 LiveKit UDP 默认端口段未迁移')

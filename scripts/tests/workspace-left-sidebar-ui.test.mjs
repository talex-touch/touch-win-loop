...
it('左栏结构大纲不再用本地推断结果充当真实大纲', async () => {
  ...
  assert.match(resourceManagerSource, /resolveOutlineCitationCount\(row\.node\) > 0/, '资源管理器未把结构来源收敛成标题后的数字引用 tag')
  assert.match(resourceManagerSource, /<Teleport to="body">/, '资源管理器未将结构来源引用浮层 teleport 到 body')
  assert.match(resourceManagerSource, /@pointerenter\.stop="showOutlineCitationTooltip\(row\.node, \$event\)"/, '资源管理器缺少结构来源引用 hover 定位入口')
  assert.match(resourceManagerSource, /workspace-outline-citation-tooltip__title/, '资源管理器缺少结构来源引用 hover 明细')
  assert.match(resourceManagerSource, /row\.node\.meta && resolveOutlineCitationCount\(row\.node\) === 0/, '资源管理器仍可能把来源数量渲染成独立第二行')
  ...
  assert.match(styleSource, /\.workspace-outline-citation-tag \{[\s\S]*cursor:\s*help;/, '左栏样式缺少结构来源数字引用 tag')
  assert.match(styleSource, /\.workspace-outline-citation-tooltip \{[\s\S]*position:\s*fixed;/, '结构引用浮层未使用 fixed 定位')
  assert.match(styleSource, /\.workspace-outline-citation-tooltip--bottom::after \{/, '结构引用浮层缺少 teleport 后的箭头定位样式')

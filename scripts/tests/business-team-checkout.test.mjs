...
it('创建 Business Team 与 Team 结算页接入模拟支付闭环', async () => {
  ...
  assert.match(switchEntry, /workspace-create-submit-button/, '创建 Team 弹窗缺少稳定提交按钮测试标识')
  assert.match(switchEntry, /writeActiveWorkspacePreference\(workspaceOption\.workspace\.id\)/, '创建支付成功后未写入 activeWorkspaceId')
  assert.match(switchEntry, /checkout: 'success'/, '创建支付成功后未带 checkout 成功态进入结算页')
  assert.match(switchEntry, /orderId: checkoutResponse\.data\.order\.id/, '创建支付成功后未把 mock orderId 传给结算页')
  assert.match(switchEntry, /created: '1'/, '创建支付成功后未标记 created 来源')
  assert.match(switchEntry, /team\/\$\{workspaceOption\.workspace\.id\}\/billing/, '创建支付成功后未跳转 Team 结算页')
  ...
  assert.match(billingPage, /data-testid="team-billing-success-state"/, 'Team 结算页缺少支付完成状态')
  assert.match(billingPage, /data-testid="team-billing-active-plan"/, 'Team 结算页缺少当前生效套餐状态')
  assert.match(billingPage, /data-testid="team-billing-next-create-project"/, 'Team 结算页缺少新建项目下一步入口')
  assert.match(billingPage, /team-billing-page__order--highlight/, 'Team 结算页未高亮当前收据')
  assert.match(billingPage, /mock provider/, 'Team 结算页收据未展示 mock provider')

...
it('评论与文档 AI 仍走现有状态链路，并保持 markdown 评论展示在文档内部右侧', async () => {
  ...
  assert.match(projectSource, /function applyAgentDocDraft\(draft: AiWorkspaceDocumentDraft\): void \{/, '项目页缺少 AgentDoc 草案写回入口')
  assert.match(projectSource, /const applied = workspaceMainPanelRef\.value\?\.applyMarkdownDocumentDraft\(draft\) \|\| false/, 'AgentDoc 草案未先经过主编辑器确认写回')
  assert.match(projectSource, /statusLine\.value = 'AgentDoc 草案已应用到文档。'/, 'AgentDoc 草案写回成功后未更新状态提示')
...
it('agentDoc 继续复用 document_assist 模式与文档场景路由', async () => {
  ...
  assert.match(projectSource, /const projectResolvedAiMode = computed<WorkspacePrimaryAiMode>\(\(\) => \{[\s\S]*projectContextualAssistant\.value\?\.aiMode \|\| 'dialog_ask'/, '项目页未将 AgentDoc 继续挂在 contextual document_assist 链路上')
  assert.match(projectSource, /if \(aiMode\.value === 'document_assist'\)\s+return 'AgentDoc'/, '项目页未将 document_assist 模式标签统一为 AgentDoc')
  assert.match(projectSource, /assistantPreset: currentWorkspaceAssistantContext\.value\.assistantPreset/, 'AgentDoc 请求上下文未透传 assistantPreset')
  assert.match(projectSource, /assistantLabel: currentWorkspaceAssistantContext\.value\.assistantLabel/, 'AgentDoc 请求上下文未透传 assistantLabel')
  assert.match(projectSource, /previewMode: currentWorkspaceAssistantContext\.value\.previewMode/, 'AgentDoc 请求上下文未透传 previewMode')
  assert.match(projectSource, /resourcePurpose: currentWorkspaceAssistantContext\.value\.resourcePurpose/, 'AgentDoc 请求上下文未透传 resourcePurpose')
  assert.match(projectSource, /assistantMetadata = \{[\s\S]*agentDocDraft: result\.documentDraft[\s\S]*workflowDraft: result\.workflowDraft[\s\S]*\}/, '项目页未把 AgentDoc 草案写入 assistant metadata')
  assert.match(sidebarSource, /label: 'AgentDoc'/, '右栏未统一显示 AgentDoc 名称')
  assert.match(sidebarSource, /workspace-agent-doc-card/, '右栏未渲染 AgentDoc 草案卡')

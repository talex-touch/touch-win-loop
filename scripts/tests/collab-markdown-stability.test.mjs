...
it('agentDoc 继续复用 document_assist 模式与文档场景路由', async () => {
  ...
  assert.match(projectSource, /assistantMetadata = result\.documentDraft\s*\?\s*\{\s*agentDocDraft: result\.documentDraft\s*\}\s*:\s*undefined/, '项目页未把 AgentDoc 草案写入 assistant metadata')

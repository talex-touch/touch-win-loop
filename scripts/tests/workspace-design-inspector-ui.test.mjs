...
it("设计属性面板改为命令栏 + 持久属性分组，并避免继续渲染假控件", async () => {
  ...
  assert.match(source, /device_mockup 渲染单图模板，device_artboard 渲染自身内容。/, "Inspector 未更新设备类型语义说明");
  assert.match(
    source,
  ...
  assert.doesNotMatch(
    source,
    /联动源画板|联动画板/,
    "Inspector 仍保留已废弃的 linked source 控件",
  );
});
...
it("Diagram 编辑态接入画布 AI 生成、补全、续改入口，并保持先预览后导入", async () => {
  const [panelSource, inspectorSource] = await Promise.all([
    readFile(DESIGN_PANEL_FILE, "utf8"),
  ...
  assert.match(panelSource, /AI 生成/, "设计面板缺少 AI 生成按钮");
  assert.match(panelSource, /AI 补全/, "设计面板缺少 AI 补全按钮");
  assert.match(panelSource, /AI 续改/, "设计面板缺少 AI 续改按钮");
  assert.match(
    panelSource,
    /:disabled="canvasAiInputDisabled"/,
    "设计面板未在画布 AI 未配置时禁用输入",
  );
  assert.match(
    panelSource,
    /canvasAiPreviewPending\.value = true;/,
    "设计面板未在画布 AI 返回后标记预览待导入状态",
  );
  assert.match(
    inspectorSource,
    /复杂操作和 AI 生成\/续改仍进入 Diagram 编辑态。/,
    "Inspector 未更新 Diagram 编辑态说明",
  );
});

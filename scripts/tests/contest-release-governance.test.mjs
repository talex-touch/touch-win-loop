...
describe('赛事版本流与前台可见性收口', () => {
  ...
  it('公开赛事页遵守竞赛库与赛道库字段边界', async () => {
    const [listPageSource, detailPageSource, contestStoreSource, domainSource, publicCoverApiSource] = await Promise.all([
      readSource('app/pages/contests/index.vue'),
      readSource('app/pages/contests/[id].vue'),
      readSource('server/utils/contest-store.ts'),
      readSource('shared/types/domain-legacy.ts'),
      readSource('server/api/contests/[id]/tracks/[trackId]/cover.get.ts'),
    ])
    ...
    assert.match(listPageSource, /resolveContestSummary\(contest\)/, '公开赛事列表卡片应在名称下展示比赛介绍')
    assert.match(listPageSource, /contest-card__summary-popover/, '公开赛事列表比赛介绍 hover 时应展示全文')
    assert.doesNotMatch(listPageSource, /赛道：\{\{\s*resolveTrackNames\(contest\)\s*\}\}/, '公开赛事列表卡片介绍下方不应继续展示赛道摘要')
    assert.doesNotMatch(listPageSource, /报名节点：\{\{[\s\S]*formatRegistrationNode/, '公开赛事列表卡片介绍下方不应继续展示报名节点')
    assert.match(listPageSource, /function resolveRegistrationWindow\(contest: Contest\)[\s\S]*contest\.timelines/, '公开赛事列表应从结构化 timeline 兜底报名节点')
    assert.match(listPageSource, /formatContestRegistrationWindow\(selectedContest\)/, '公开赛事弹窗关键节点应使用报名节点兜底口径')
    ...
    assert.match(listPageSource, /resolveContestDisplayId\(contest\)/, '公开赛事列表标题旁应展示竞赛业务编号')
    assert.match(listPageSource, /const fieldValuePlaceholder = '-'/, '公开赛事页字段缺失值应统一展示为 -')
    ...
    assert.match(listPageSource, /selectedContestTrackYear/, '公开赛事弹窗左侧赛道应支持年份筛选状态')
    assert.match(listPageSource, /contestTrackYearOptions/, '公开赛事弹窗左侧赛道应展示可切换年份选项')
    assert.match(listPageSource, /selectContestTrackYear/, '公开赛事弹窗左侧赛道年份切换应同步当前选中赛道')
    assert.match(listPageSource, /handleContestTrackYearChange/, '公开赛事弹窗左侧赛道年份切换应使用独立事件处理函数')
    assert.match(listPageSource, /visibleContestTracks/, '公开赛事弹窗左侧赛道列表应按年份过滤')
    assert.match(listPageSource, /resolveTrackDisplayId\(track\)/, '公开赛事页赛道名称旁应展示可追溯赛道编号')
    assert.doesNotMatch(listPageSource, /resolveTrackDisplayId[\s\S]*normalizeDisplayId\(track\.id\)/, '公开赛事页赛道编号不应回退展示内部 UUID')
    assert.match(contestStoreSource, /LEFT JOIN LATERAL \([\s\S]*scope = 'contest'[\s\S]*entity_id = contests\.id[\s\S]*LIMIT 1[\s\S]*\) contest_ref ON TRUE/, '公开赛事列表应从 Feishu external refs 带出竞赛编号')
    assert.match(contestStoreSource, /LEFT JOIN LATERAL \([\s\S]*scope = 'track'[\s\S]*entity_id = contest_tracks\.id[\s\S]*LIMIT 1[\s\S]*\) track_ref ON TRUE/, '公开赛事列表应从 Feishu external refs 带出赛道编号')
    assert.match(contestStoreSource, /mergeExternalRefMetadata\(row\.metadata, row\.external_id/, '公开赛事列表应把外部编号合并到前台 metadata')
    ...
    assert.doesNotMatch(listPageSource, /<dt>组队规则<\/dt>[\s\S]*selectedContest\.teamRule/, '公开赛事弹窗赛事级参赛信息不应展示组队规则')
    assert.match(listPageSource, /<dt>组队规则<\/dt>[\s\S]*selectedContestTrack\.teamRule/, '公开赛事弹窗赛道详情应展示赛道组队规则')
    assert.match(listPageSource, /<dt>必备项<\/dt>[\s\S]*selectedContestTrack\.evidenceRequirements/, '公开赛事弹窗赛道详情应展示必备项')
    assert.match(listPageSource, /<dt>加分项<\/dt>[\s\S]*selectedContestTrack\.scoringPoints/, '公开赛事弹窗赛道详情应展示加分项')
    assert.match(listPageSource, /<dt>扣分项<\/dt>[\s\S]*selectedContestTrack\.deductionItems/, '公开赛事弹窗赛道详情应展示扣分项')
    assert.match(domainSource, /export interface Track \{[\s\S]*evidenceRequirements\?: string\[\][\s\S]*scoringPoints\?: string\[\][\s\S]*deductionItems\?: string\[\]/, '公开 Track 类型应带出赛道规则项')
    assert.match(contestStoreSource, /COALESCE\(rubric\.scoring_points[\s\S]*COALESCE\(rubric\.deduction_items[\s\S]*COALESCE\(rubric\.evidence_requirements/, '公开赛事列表应从 rubric 带出赛道规则项')
    ...
    assert.match(listPageSource, /selectedTrackBannerSource/, '公开赛事弹窗赛道详情应展示赛道封面 banner')
    assert.match(listPageSource, /function parseTrackCodeYear\(value: unknown\)[\s\S]*\^\\d\{4\}\(\\d\{2\}\)\\d\{8\}\$/, '公开赛事页应从赛道编号解析年份')
    assert.match(listPageSource, /const trackCodeYear = parseTrackCodeYear\(resolveTrackDisplayId\(track\)\)[\s\S]*if \(trackCodeYear\)\s+return \[trackCodeYear\]/, '公开赛事页赛道编号年份应优先于时间线年份')
    assert.match(listPageSource, /resolveImagePreviewSource\(selectedContestTrack\.value\.coverImageUrl,\s*\{[\s\S]*contestId: selectedContest\.value\.id[\s\S]*trackId: selectedContestTrack\.value\.id/, '公开赛事弹窗赛道封面应按公开赛事与赛道上下文解析可预览地址')
    assert.match(listPageSource, /isFeishuAdminAttachmentPreviewSource/, '公开赛事弹窗不应直接裸渲染后台飞书附件预览地址')
    assert.match(listPageSource, /buildPublicTrackCoverPreviewSource/, '公开赛事弹窗应把飞书附件封面改写到公开赛道封面代理')
    assert.match(listPageSource, /@error="markTrackBannerFailed\(selectedTrackBannerSource\)"/, '公开赛事弹窗赛道封面加载失败时应移除破图')
    assert.match(listPageSource, /selectedTrackBannerUnavailable/, '公开赛事弹窗赛道封面失败时应显示稳定占位')
    assert.match(publicCoverApiSource, /contest_tracks\.cover_image_url/, '公开赛道封面代理应只从已落库赛道封面读取')
    assert.match(publicCoverApiSource, /contests\.status = 'published'[\s\S]*contests\.visibility = 'public'[\s\S]*contest_tracks\.status = 'published'/, '公开赛道封面代理必须限制公开已发布赛事与赛道')
    assert.match(publicCoverApiSource, /FROM release_versions rv[\s\S]*rv\.scope_kind = 'contest'[\s\S]*rv\.status = 'published'[\s\S]*rv\.live_entity_id = contests\.id/, '公开赛道封面代理必须沿用公开赛事发布版本口径')
    assert.match(publicCoverApiSource, /extractFeishuAttachmentToken/, '公开赛道封面代理应从受控飞书附件预览地址提取 token')
    assert.match(publicCoverApiSource, /downloadFeishuDriveMedia/, '公开赛道封面代理应代理下载飞书图片内容')
    assert.match(publicCoverApiSource, /media\.contentType\.startsWith\('image\/'\)/, '公开赛道封面代理应拒绝非图片附件')
    ...
    assert.match(listPageSource, /function isReliableTrackTimelineYear[\s\S]*parser_current_year[\s\S]*fallback_current_year/, '公开赛事页不应把当前年份解析兜底展示成真实赛道年份')
    assert.match(listPageSource, /formatTimelineDisplay\(item\)/, '公开赛事弹窗赛道时间节点应在无日期时展示原始节点说明')
    assert.match(listPageSource, /function sanitizePublicTimelineText\(value: unknown\)[\s\S]*飞书同步[\s\S]*derived:\)\?\(\?:contest\|track\):/, '公开赛事弹窗应清洗时间节点内部同步标记')
    assert.match(listPageSource, /const direct = sanitizePublicTimelineText\(label\)/, '公开赛事弹窗时间节点标题不应展示飞书同步派生 ID')
    assert.match(listPageSource, /const note = sanitizePublicTimelineText\(item\.note\)/, '公开赛事弹窗时间节点说明不应展示飞书同步前缀')
    assert.match(listPageSource, /selectedTrackResources/, '公开赛事弹窗赛道详情应展示按赛道匹配的资料')
    assert.match(listPageSource, /resolveDisplayTrackResources/, '公开赛事弹窗赛道资料为空时应提供临时 mock 资料')
    assert.match(listPageSource, /buildMockTrackResources/, '公开赛事弹窗赛道资料 mock 应集中生成')
    assert.match(listPageSource, /参赛信息整理中/, '公开赛事弹窗赛道资料兜底应使用面向用户的整理中文案')
    assert.match(listPageSource, /提交要求整理中/, '公开赛事弹窗赛道资料兜底应使用提交要求整理中文案')
    assert.doesNotMatch(listPageSource, /占位资料|模板占位|待资料库导入|飞书资料库同步/, '公开赛事弹窗赛道资料兜底不应暴露内部导入或占位口径')
    assert.doesNotMatch(listPageSource, />Mock</, '公开赛事弹窗不应向用户展示 mock 标识')
    assert.doesNotMatch(listPageSource, /isMockResource\(resource\)/, '公开赛事弹窗不应用 mock 来源渲染前台徽标')
    assert.match(listPageSource, /resolveDisplayTrackResources\(selectedContest\.value, selectedContestTrack\.value, selectedContestTrackYear\.value\)/, '公开赛事弹窗赛道资料应按当前年份过滤并补 mock 兜底')
    assert.match(listPageSource, /resolveTrackResources\(contest, track, selectedYear\)/, '公开赛事弹窗赛道资料真实数据仍应按当前年份过滤')
    assert.doesNotMatch(listPageSource, /selectedTrackFaqItems/, '公开赛事弹窗赛道详情不应展示 FAQ')
    assert.doesNotMatch(listPageSource, /暂无赛道常见疑问/, '公开赛事弹窗赛道详情不应展示 FAQ 空态')
    ...
    assert.match(listPageSource, /\.contest-detail-modal__tab-shell[\s\S]*grid-template-columns: 220px minmax\(0, 1fr\)/, '公开赛事弹窗详情应使用左侧 tabs 与右侧内容区')
    assert.match(listPageSource, /\.contest-detail-modal__tabs[\s\S]*overflow-y: auto/, '公开赛事弹窗左侧 tree 应支持内部滚动')
    assert.match(listPageSource, /\.contest-detail-modal__tab-panel[\s\S]*overflow-y: auto/, '公开赛事弹窗 tab 内容区应内部滚动')
    assert.match(listPageSource, /resolveTextTags\(selectedContest\.keywords\)/, '公开赛事弹窗关键词应使用紧凑标签展示')
    assert.match(listPageSource, /contest-detail-modal__title-row[\s\S]*contest-detail-modal__meta/, '公开赛事弹窗级别和学科标签应放在标题右侧')
    assert.match(listPageSource, /resolveLevelIcon\(selectedContest\.level\)/, '公开赛事弹窗级别标签应展示图标')
    assert.match(listPageSource, /\.contest-detail-modal__meta > span/, '公开赛事弹窗 meta 标签样式不应误伤级别图标')
    assert.match(listPageSource, /contest-detail-modal__level-badge--national[\s\S]*contest-detail-modal__level-badge--provincial[\s\S]*contest-detail-modal__level-badge--school[\s\S]*contest-detail-modal__level-badge--industry/, '公开赛事弹窗级别标签应按级别区分颜色')
    assert.doesNotMatch(listPageSource, /contest-detail-modal__nav-item\.is-active[\s\S]*box-shadow/, '公开赛事弹窗左侧 tabs 选中态不应使用阴影')
    assert.doesNotMatch(listPageSource, /待补充 <strong>\{\{\s*resolveMissingFieldCount\(contest\)\s*\}\}<\/strong>/, '公开赛事列表不应继续展示不可解释的待补充数量')

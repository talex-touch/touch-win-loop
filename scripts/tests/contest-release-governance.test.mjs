...
describe('赛事版本流与前台可见性收口', () => {
  ...
  it('赛道确认表单不会把裸附件文件名当图片地址，并补充多口径时间节点关联', async () => {
    ...
    assert.match(workbenchSource, /function trackFormRows[\s\S]*snapshot\?\.trackTimelines[\s\S]*item\.timelineText[\s\S]*function reviewLogPayloadText/, '赛道确认表单应在赛道维度展示结构化时间节点并用原始 timelineText 兜底')
  })

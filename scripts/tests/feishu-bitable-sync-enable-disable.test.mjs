...
it('自动同步规则会在类型、存储、接口、执行链路和编辑器中生效', async () => {
  ...
  assert.match(serviceSource, /status: toText\(fields\.status\) \|\| \(normalizedAutoSync\.enabled \? normalizedAutoSync\.syncStatusField : ''\)/, '自动同步规则未为回填状态字段提供同步信息兜底')
  ...
  assert.match(editorSource, /function openAutoSyncDrawer\(\) \{[\s\S]*autoSyncDrawerVisible\.value = true/, '编辑器未提供自动同步 Drawer 打开入口')

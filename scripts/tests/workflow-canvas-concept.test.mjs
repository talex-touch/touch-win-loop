...
it('项目页从流程入口打开 workflow 画布，并切到 draw.io + AgentProto 工作台', async () => {
  ...
  assert.match(flowTabSource, /draw\.io/)
  assert.match(flowTabSource, /AgentProto/)
  assert.match(flowTabSource, /pointer-events-none absolute right-3 top-3 z-10 items-center/)
...
it('项目页对 AgentProto workflow 草案使用独立动作 gating，并在恢复消息时重置本地 workflow 草案状态', async () => {
  ...
  assert.match(pageSource, /const workflowGenerateAvailable = computed\(\(\) => isAiFeatureAvailable\(resolveWorkflowDraftFeatureKey\('generate'\)\)\)/)
  assert.match(pageSource, /const workflowCompleteAvailable = computed\(\(\) => isAiFeatureAvailable\(resolveWorkflowDraftFeatureKey\('complete'\)\)\)/)
  assert.match(pageSource, /const workflowRefineAvailable = computed\(\(\) => isAiFeatureAvailable\(resolveWorkflowDraftFeatureKey\('refine'\)\)\)/)
  assert.match(pageSource, /const workflowRestyleAvailable = computed\(\(\) => isAiFeatureAvailable\(resolveWorkflowDraftFeatureKey\('restyle'\)\)\)/)
})

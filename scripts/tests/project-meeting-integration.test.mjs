...
it('会议领域模型、数据库表与实时事件已接入项目工作区', async () => {
  const [domainSource, dbSource, realtimeSource, realtimeHubSource, realtimeComposableSource] = await Promise.all([
  ...
  assert.match(domainSource, /export interface ProjectMeetingGuestShare \{/, '缺少 ProjectMeetingGuestShare 领域模型')
  assert.match(domainSource, /export interface SharedProjectMeetingSnapshot \{/, '缺少 SharedProjectMeetingSnapshot 领域模型')
  assert.match(domainSource, /export interface ProjectMeetingGuestJoinSession \{/, '缺少 ProjectMeetingGuestJoinSession 领域模型')
  ...
  assert.match(dbSource, /CREATE TABLE IF NOT EXISTS project_meeting_guest_shares\b/, '缺少 project_meeting_guest_shares 表')
...
it('项目页已接入会议创建页、详情页、start\/join 链路与 tab 恢复逻辑', async () => {
  ...
  assert.match(source, /async function createProjectMeetingGuestShare\(meetingId: string\): Promise<void> \{/, '项目页缺少生成 guest share 函数')
  assert.match(source, /async function regenerateProjectMeetingGuestShare\(meetingId: string\): Promise<void> \{/, '项目页缺少重建 guest share 函数')
  assert.match(source, /async function revokeProjectMeetingGuestShare\(meetingId: string\): Promise<void> \{/, '项目页缺少撤销 guest share 函数')

...
it('项目页已接入会议创建页、详情页、start\/join 链路与 tab 恢复逻辑', async () => {
  ...
  assert.match(meetingComposableSource, /const projectMeetingsLoadedProjectId = ref\(''\)/, '会议 composable 缺少项目级加载快照')
  assert.match(meetingComposableSource, /const meetingDetailLoadedId = ref\(''\)/, '会议 composable 缺少详情级加载快照')
  assert.match(meetingComposableSource, /const meetingUtterancesLoadedId = ref\(''\)/, '会议 composable 缺少逐句稿级加载快照')
  assert.match(meetingComposableSource, /const sameProjectRefresh = projectMeetingsLoadedProjectId\.value === projectId/, '会议 composable 未区分同项目后台刷新')
  assert.match(meetingComposableSource, /const sameMeetingRefresh = meetingDetailLoadedId\.value === targetMeetingId/, '会议 composable 未区分同会议后台刷新')
  assert.match(meetingComposableSource, /const sameMeetingRefresh = meetingUtterancesLoadedId\.value === targetMeetingId/, '会议逐句稿加载未区分同会议后台刷新')
  assert.match(meetingComposableSource, /&& !sameMeetingRefresh\) \{\s+activeMeetingUtterances\.value = \[\]\s+meetingUtterancesLoadedId\.value = ''/s, '会议逐句稿在同会议刷新失败时仍会清空旧内容')
  assert.match(source, /const projectMeetingsFirstLoadLoading = computed\(\(\) => \{/, '项目页缺少会议列表首屏加载态计算')
  assert.match(source, /const projectMeetingsRefreshing = computed\(\(\) => \{/, '项目页缺少会议列表后台刷新态计算')
  assert.match(source, /const meetingDetailDisplayLoading = computed\(\(\) => \{/, '项目页缺少会议详情首屏加载态计算')
  assert.match(source, /const meetingDetailRefreshing = computed\(\(\) => \{/, '项目页缺少会议详情后台刷新态计算')
  assert.match(source, /:meeting-loading="projectMeetingsFirstLoadLoading"/, '项目页未向会议相关组件透传首屏加载态')
  assert.match(source, /:meeting-refreshing="projectMeetingsRefreshing"/, '项目页未向会议相关组件透传后台刷新态')
  assert.match(source, /:meeting-detail-loading="meetingDetailDisplayLoading"/, '项目页未向会议详情透传首屏加载态')
  assert.match(source, /:meeting-detail-refreshing="meetingDetailRefreshing"/, '项目页未向会议详情透传后台刷新态')
})

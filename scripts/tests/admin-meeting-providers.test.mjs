...
it('admin meeting providers 页面支持测试连通性并展示 rtc/asr 探针结果', async () => {
  ...
  assert.match(source, /monitoringPrometheusBaseUrl/, '会议服务配置页未提供 Prometheus 系统配置项')
  assert.match(source, /Prometheus Base URL/, '会议服务配置页未展示监控配置入口')
...
it('admin meeting providers API 复用公共配置合并逻辑并校验写权限', async () => {
  ...
  assert.match(configSource, /prometheusBaseUrl/, '会议服务后台未支持监控配置合并')

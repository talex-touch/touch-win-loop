...
describe('workspace Feishu production diagnostics contracts', () => {
  ...
  it('exposes audits API and connector diagnostics panel', async () => {
    ...
    assert.match(panelSource, /健康诊断/, '连接器面板缺少健康诊断区')
    assert.match(panelSource, /最近同步/, '连接器面板缺少最近同步区')
    assert.match(panelSource, /最近导入/, '连接器面板缺少最近导入区')
    assert.match(panelSource, /审计日志/, '连接器面板缺少审计日志区')
    assert.match(panelSource, /diagnosticSummary/, '连接器面板未消费诊断 summary')
    assert.match(panelSource, /auditLogs/, '连接器面板未展示审计日志')
    assert.doesNotMatch(panelSource, /自动登录/, '连接器面板不应展示自动登录结果')
  })
  ...
  it('provides a real-browser Feishu diagnostics smoke entry', async () => {
    ...
    assert.match(smokeSource, /button\.user-settings-tab[\s\S]*连接器/, 'smoke 未进入连接器 tab')
    assert.match(smokeSource, /user-settings-third-party-platforms-panel/, 'smoke 未验证连接器面板')
    assert.match(smokeSource, /健康诊断[\s\S]*最近同步[\s\S]*最近导入[\s\S]*审计日志/, 'smoke 未覆盖诊断核心区域')

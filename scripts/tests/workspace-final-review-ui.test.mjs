...
it('终审工作台已接入真实项目视频通话链路', async () => {
  const [workspaceSource, workbenchSource] = await Promise.all([
    readFile(WORKSPACE_DETAIL_FILE, 'utf8'),
    readFile(FINAL_REVIEW_WORKBENCH_FILE, 'utf8'),
  ])

  assert.match(workbenchSource, /data-testid="workspace-final-review-video-call-action"/, '终审工作台缺少视频通话入口')
  assert.match(workbenchSource, /接入视频通话/, '终审视频入口文案未使用正式功能语义')
  assert.match(workbenchSource, /emit\('openVideoCall'\)/, '终审视频入口未暴露 openVideoCall 事件')
  assert.match(workspaceSource, /const finalReviewVideoMeeting = computed\(\(\) => \{[\s\S]*projectMeetings\.value[\s\S]*item\.mode === 'video'[\s\S]*终审\|评审\|review/, '项目页未从真实会议列表推导终审视频会')
  assert.match(workspaceSource, /async function openVideoCallFromFinalReview\(\): Promise<void> \{[\s\S]*await loadProjectMeetings\(\{ fallbackToFirst: false, hydrateSelectedDetail: false \}\)[\s\S]*await joinProjectMeeting\(meeting\.id\)[\s\S]*await startProjectMeeting\(meeting\.id\)[\s\S]*await submitProjectMeetingCreate\(\{[\s\S]*mode: 'video'[\s\S]*title: '终审视频通话'/, '终审视频入口未复用项目会议的加载、加入、启动和创建链路')
  assert.match(workspaceSource, /@open-video-call="openVideoCallFromFinalReview"/, '终审驾驶舱未绑定视频通话处理函数')
  assert.doesNotMatch(workbenchSource, /mockup/i, '终审视频通话 UI 不应出现 mockup 字样')
})

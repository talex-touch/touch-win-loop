...
it('竞赛库映射会收敛到实际需要的字段并同步收窄默认模板与预检展示', async () => {
  ...
  assert.ok(previewFocusBlock, '未找到预检重点字段定义')
  assert.ok(contestDefaultConfigBlock, '未找到竞赛默认模板定义')
  assert.doesNotMatch(contestMappingBlock, /organizer（主办方）/, '竞赛库映射仍然暴露主办方字段')
  assert.doesNotMatch(contestMappingBlock, /coOrganizer（协办方）/, '竞赛库映射仍然暴露协办方字段')
  assert.doesNotMatch(contestMappingBlock, /participantRequirements（参赛对象）/, '竞赛库映射仍然暴露参赛对象字段')
  assert.doesNotMatch(contestMappingBlock, /teamRule（组队规则）/, '竞赛库映射仍然暴露组队规则字段')
  assert.doesNotMatch(contestMappingBlock, /currentSeason（届次）/, '竞赛库映射仍然暴露届次字段')
  assert.doesNotMatch(contestMappingBlock, /aliases（别名）/, '竞赛库映射仍然暴露别名字段')
  assert.doesNotMatch(contestMappingBlock, /recommendedFor（推荐人群）/, '竞赛库映射仍然暴露推荐人群字段')
  assert.match(contestMappingBlock, /registrationWindow（报名时间）/, '竞赛库映射缺少报名时间字段')
  assert.match(contestMappingBlock, /submissionDeadline（截止时间）/, '竞赛库映射缺少截止时间字段')
  assert.match(mappingAliasBlock, /registrationWindow:\s*\[[\s\S]*报名时间[\s\S]*报名窗口[\s\S]*\]/, '竞赛库映射缺少报名时间字段猜测')
  assert.match(mappingAliasBlock, /submissionDeadline:\s*\[[\s\S]*截止时间[\s\S]*提交截止时间[\s\S]*提交时间[\s\S]*\]/, '竞赛库映射缺少截止时间字段猜测')
  assert.match(previewFocusBlock, /return \['externalId', 'name', 'officialUrl', 'registrationWindow', 'submissionDeadline'\]/, '竞赛库预检重点字段未纳入时间字段')
  assert.match(contestDefaultConfigBlock, /fieldMap:\s*\{[\s\S]*name:\s*''[\s\S]*officialUrl:\s*''[\s\S]*summary:\s*''[\s\S]*level:\s*''[\s\S]*disciplines:\s*''[\s\S]*keywords:\s*''[\s\S]*registrationWindow:\s*''[\s\S]*submissionDeadline:\s*''[\s\S]*\}/, '竞赛库默认模板未收敛到精简字段并补齐时间字段')
  assert.doesNotMatch(contestDefaultConfigBlock, /organizer:\s*''/, '竞赛库默认模板仍然包含主办方')
  assert.doesNotMatch(contestDefaultConfigBlock, /recommendedFor:\s*''/, '竞赛库默认模板仍然包含推荐人群')

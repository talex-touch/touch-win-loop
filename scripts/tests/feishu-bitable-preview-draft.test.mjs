...
describe('飞书多维表格版本草稿链路', () => {
  ...
  it('资料库、政策库与人设库映射已经切到当前字段集，并打通新实体链路', async () => {
    ...
    assert.match(componentSource, /judgeType（评委类型）/, '人设映射缺少评委类型字段')
    assert.match(componentSource, /systemPrompt（系统提示词）/, '人设映射缺少系统提示词字段')
    assert.match(configSource, /if \(entityType === 'policy'\) \{[\s\S]*meetingName:\s*''[\s\S]*xiaohongshuMaterialLink:\s*''/, '政策默认模板未补齐完整字段')
    assert.match(configSource, /if \(entityType === 'persona'\) \{[\s\S]*contestExternalIdField:\s*''[\s\S]*judgeType:\s*''[\s\S]*systemPrompt:\s*''/, '人设默认模板未补齐关键字段')
    ...
    assert.match(serviceSource, /getDefensePersonaPresetByExternalId/, '人设预检未基于预设存量判断 create\/update')

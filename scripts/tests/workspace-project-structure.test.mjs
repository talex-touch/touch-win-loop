...
it('项目工作区入口已将 route、侧栏布局、领域状态机与纯 helper 拆出页面脚本', async () => {
  ...
  assert.match(sidebarLayoutSource, /const leftSidebarWidth = ref\(normalizeWorkspaceLeftSidebarWidth\(null\)\)/, '项目工作区左栏默认宽度未初始化')
  assert.match(sidebarLayoutSource, /const rightSidebarUserCollapsed = ref\(true\)/, '项目工作区右栏默认未收起')
  assert.match(sidebarLayoutSource, /const rightSidebarWidth = ref\(normalizeWorkspaceRightSidebarWidth\(null\)\)/, '项目工作区右栏默认宽度未初始化')
  ...
  assert.match(shellSource, /leftSidebarWidth: normalizeWorkspaceLeftSidebarWidth\(null\),\s+rightSidebarWidth: normalizeWorkspaceRightSidebarWidth\(null\),\s+leftSidebarCollapsed: true,\s+rightSidebarCollapsed: true,/, '项目工作区默认视图状态未初始化左右栏宽度与双侧收起态')

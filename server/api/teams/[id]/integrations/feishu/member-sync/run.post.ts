...
import { runFeishuWorkspaceMemberSync } from '~~/server/utils/workspace-integration-store'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const candidates = await withClient(event, async (db) => {
      ...
      return resolveFeishuWorkspaceMemberSyncCandidates(event, db, {
        workspaceId,
        actorUserId: user.id,
    ...
    const data = await withTransaction(event, async (db) => {
      ...
      if (!canManage)
        throw new Error('FORBIDDEN')
      return runFeishuWorkspaceMemberSync(db, {
        workspaceId,
        candidates,
        actorUserId: user.id,
      })
    ...
    if (error instanceof Error && error.message === 'FORBIDDEN') {
      ...
      return fail('仅工作空间 owner/admin 可执行飞书成员同步。', {
        startedAt,
        provider: runtime.ai.provider,
        model: runtime.ai.model,
      }, 40400)

...
import { getWorkspaceIntegrationList } from '~~/server/utils/workspace-integration-store'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const data = await withClient(event, async (db) => {
      ...
      return getWorkspaceIntegrationList(db, workspaceId)
    })
    ...
    if (error instanceof Error && error.message === 'FORBIDDEN') {
      ...
      return fail('当前用户无权查看该工作空间第三方平台。', {
        startedAt,
        provider: runtime.ai.provider,
        model: runtime.ai.model,
      }, 40394)

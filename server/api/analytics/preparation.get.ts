import type { AnalyticsFilterInput, AnalyticsPreparationCadencePayload } from '~~/shared/types/analytics'
import { getQuery } from 'h3'
import { getPreparationCadenceAnalysis } from '~~/server/utils/analytics-store'
...
export default defineEventHandler(async (event) => {
  ...
  const payload = await withClient(event, db => getPreparationCadenceAnalysis(db, {
    user,
    includeInternal,
    filters,
  }))

  return ok<AnalyticsPreparationCadencePayload>(payload, {
    startedAt,

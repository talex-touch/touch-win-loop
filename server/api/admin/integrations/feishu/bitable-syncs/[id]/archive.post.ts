...
import { archiveFeishuBitableSync } from '~~/server/utils/feishu-integration-store'
...
export default defineEventHandler(async (event) => {
  ...
  const sync = await withTransaction(event, async (db) => {
    return archiveFeishuBitableSync(db, {
      actorUserId: user.id,
      syncId,
    })
  }).catch((error) => {
    setResponseStatus(event, 400)
    return fail(error instanceof Error ? error.message : '同步信息归档失败。', {
      startedAt,
      provider: runtime.ai.provider,
      model: runtime.ai.model,
      fallbackUsed: false,

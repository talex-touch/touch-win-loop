...
import { revokeProjectMeetingGuestShareByHost } from '~~/server/services/meeting/project-meeting'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const share = await withTransaction(event, async (db) => {
      ...
      return revokeProjectMeetingGuestShareByHost(db, {
        projectId,
        meetingId,
        user,
      })

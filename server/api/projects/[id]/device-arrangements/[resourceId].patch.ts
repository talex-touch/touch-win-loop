...
import { updateProjectDeviceArrangement } from '~~/server/utils/project-resource-device-arrangement-store'
...
export default defineEventHandler(async (event): Promise<ApiResponse<{
  ...
  if (!projectId || !resourceId || !body.document) {
    setResponseStatus(event, 400)
    return fail('缺少 projectId、resourceId 或 document。', {
      startedAt,
  ...
  try {
    const updated = await withTransaction(event, async (db) => {
      ...
      return { ...result, workspaceId: access.workspaceId }
    })

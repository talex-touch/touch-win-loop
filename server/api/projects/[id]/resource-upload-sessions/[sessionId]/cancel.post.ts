...
export default defineEventHandler(async (event) => {
  ...
  const result = await withTransaction(event, async (db) => {
    ...
    if (!access.ok)
      return { ok: false as const, reason: access.reason, session: null, chunkKeys: [] as string[] }

    const chunkRows = await listProjectResourceUploadChunkRows(db, { sessionId })
    const session = await updateProjectResourceUploadSessionStatus(db, {
      projectId,
      sessionId,
      fromStatuses: ['queued', 'uploading', 'paused', 'failed', 'finalizing'],
      toStatus: 'canceled',
    })
    if (!session)
      return { ok: false as const, reason: 'SESSION_NOT_FOUND' as const, session: null, chunkKeys: [] as string[] }

    return {
      ok: true as const,
      session,
      chunkKeys: chunkRows.map(row => normalizeString(row.object_key)).filter(Boolean),
    }
  })
  ...
  await deleteObjectsAcrossStorageChannels(result.chunkKeys, runtime).catch(() => undefined)

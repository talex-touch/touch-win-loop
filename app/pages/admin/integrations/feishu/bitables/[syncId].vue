...
const selectedItemId = computed(() => String(route.query.item || '').trim())
...
function applyDraftToNewItemForm() {
  newItemForm.tableId = String(route.query.draftTableId || '').trim()
  newItemForm.viewId = String(route.query.draftViewId || '').trim()
...
async function loadSyncDetail() {
  ...
  clearFeedback()
  try {
    const response = await $fetch<ApiResponse<FeishuBitableSyncDetail>>(endpoint(`/admin/integrations/feishu/bitable-syncs/${encodeURIComponent(syncId.value)}?includeInactive=true`))
    ...
    await loadTables()
    const nextItemId = selectedItemId.value && response.data.items.some(item => item.id === selectedItemId.value)
...
async function loadItemDetail(itemId: string) {
  if (!syncId.value || !itemId)
    return
  loadingItem.value = true
  try {
    const response = await $fetch<ApiResponse<FeishuBitableSyncItemDetail>>(endpoint(`/admin/integrations/feishu/bitable-syncs/${encodeURIComponent(syncId.value)}/items/${encodeURIComponent(itemId)}?runLimit=20&issueLimit=50`))
...
async function saveCurrentItem() {
  if (!syncId.value || !currentItem.value)
    return
  ...
  try {
    await $fetch(endpoint(`/admin/integrations/feishu/bitable-syncs/${encodeURIComponent(syncId.value)}/items/${encodeURIComponent(currentItem.value.id)}`), {
      method: 'PATCH',
      body: {
...
async function saveSyncInfo() {
  ...
  clearFeedback()
  try {
    const response = await $fetch<ApiResponse<FeishuBitableSync>>(endpoint(`/admin/integrations/feishu/bitable-syncs/${encodeURIComponent(syncId.value)}`), {
      method: 'PATCH',
      body: {
        name,
      },
...
async function previewCurrentItem() {
  ...
  clearFeedback()
  try {
    const response = await $fetch<ApiResponse<FeishuBitableSyncItemPreviewResult>>(endpoint(`/admin/integrations/feishu/bitable-syncs/${encodeURIComponent(syncId.value)}/items/${encodeURIComponent(currentItem.value.id)}/preview`), {
      method: 'POST',
    })
...
async function runCurrentItem() {
  ...
  clearFeedback()
  try {
    await $fetch(endpoint(`/admin/integrations/feishu/bitable-syncs/${encodeURIComponent(syncId.value)}/items/${encodeURIComponent(currentItem.value.id)}/run`), {
      method: 'POST',
    })
    await loadSyncDetail()
    setSuccess('同步执行完成。')
...
async function createItem() {
  ...
  try {
    ...
    const response = await $fetch<ApiResponse<FeishuBitableSyncItem>>(endpoint(`/admin/integrations/feishu/bitable-syncs/${encodeURIComponent(syncId.value)}/items`), {
      method: 'POST',
      body: {
...
watch(selectedItemId, (value) => {
  ...
  if (itemId && syncDetail.value?.items.some(item => item.id === itemId))
    ...
    <div class="flex flex-wrap gap-3 items-start justify-between">
      <div class="space-y-1">
        <div class="flex gap-2 items-center">
          <a-button size="small" @click="navigateTo('/admin/integrations/feishu')">
            返回列表
          </a-button>
          ...
          <a-spin :loading="loadingSync">
            <div v-if="syncDetail?.items.length" class="space-y-2">
              <button
                ...
                :class="selectedItemId === item.id ? 'border-sky-500 bg-sky-50' : 'border-slate-200 bg-white hover:border-slate-300'"

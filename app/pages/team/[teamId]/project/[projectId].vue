...
import type {
  CollabSnapshotPayload,
  WorkspaceRealtimeEnvelope,
} from "~/composables/useCollabSession";
import type {
  MappingTone,
  WorkspaceFormState,
  WorkspaceKeyword,
...
function createProjectCommonFormFromProject(
  project: Project | null,
): WorkspaceProjectCommonForm {
  if (!project) return createEmptyProjectCommonForm();
...
function cloneProjectCommonForm(
  value: WorkspaceProjectCommonForm,
): WorkspaceProjectCommonForm {
  return {
    title: value.title,
...
function createEmptyProjectAdaptationForm(
  contestId = "",
  trackId = "",
): WorkspaceProjectAdaptationForm {
  return {
...
function cloneProjectAdaptationForm(
  value: WorkspaceProjectAdaptationForm,
): WorkspaceProjectAdaptationForm {
  return {
...
function cloneProjectContestBindings(
  value: WorkspaceProjectContestBindingForm[],
): WorkspaceProjectContestBindingForm[] {
...
function defaultAssistantGreeting(): ChatMessage {
  return {
    role: "assistant",
    content:
      "你好，我是 WinLoop AI。先在左侧筛选竞赛，再告诉我你想做的项目方向，我会帮你生成可落地草案。",
  };
}
...
async function ensureCanonicalWorkspaceProjectRoute(): Promise<boolean> {
  if (!route.path.startsWith("/workspace/")) return false;
...
function parseFileSizeFromResource(resource: Resource): number {
  const sourceType = String(
    resource.sourceType || resource.source || "",
  ).trim();
  if (sourceType !== "project_upload" && sourceType !== "upload") return 0;
...
function validateUploadFiles(files: File[], usedBytes: number): string | null {
  if (!files.length) return "未检测到可上传文件。";
  ...
  const oversizeFile = files.find(
    (file) => file.size > PROJECT_RESOURCE_UPLOAD_MAX_FILE_SIZE_BYTES,
  );
  if (oversizeFile) {
...
function resolveApiStatusCode(error: unknown): number {
  ...
  const dataStatus = Number(
    (error as { data?: { statusCode?: number } }).data?.statusCode || 0,
  );
  if (Number.isFinite(dataStatus) && dataStatus > 0) return dataStatus;
...
const PROJECT_SETTINGS_DRAFT_DEVICE_PREFIX =
  "workspace.projectSettingsDraftDevice";
const RIGHT_SIDEBAR_BREAKPOINT_QUERY = "(min-width: 1280px)";
const WORKSPACE_MEMBER_MANAGE_ROLES: WorkspaceMemberRole[] = [
  "owner",
  "admin",
  "manager",
];
...
const highlightedProjectId = computed(
  () => routeProjectId.value || normalizeQueryParam(route.query.projectId),
);
...
const projectSettingsCommon = reactive<WorkspaceProjectCommonForm>(
...
const projectSettingsAdaptation = reactive<WorkspaceProjectAdaptationForm>(
  createEmptyProjectAdaptationForm(),
);
const projectSettingsAdaptationDrafts = ref<
  Record<string, WorkspaceProjectAdaptationForm>
>({});
...
const rightSidebarCollapsed = computed(
  () => rightSidebarUserCollapsed.value || rightSidebarAutoCollapsed.value,
);
...
function applyRightSidebarAutoCollapse(
  nextCollapsed: boolean,
  nextRestorePending = rightSidebarAutoRestorePending.value,
): void {
...
function initializeRightSidebarBreakpointTracking(): void {
  ...
  rightSidebarBreakpointMediaQuery = window.matchMedia(
    RIGHT_SIDEBAR_BREAKPOINT_QUERY,
  );
  ...
  if (typeof rightSidebarBreakpointMediaQuery.addEventListener === "function") {
    ...
    unsubscribeRightSidebarBreakpoint = () => {
      rightSidebarBreakpointMediaQuery?.removeEventListener(
        "change",
        handleChange,
      );
...
function setRightSidebarUserCollapsed(
  nextCollapsed: boolean,
...
function readProjectSettingsDraftCache(
  projectId: string,
...
function writeProjectSettingsDraftCache(
  projectId: string,
...
const contestSource = computed(() => {
  return contestCatalog.value.length > 0
    ? contestCatalog.value
    : contests.value;
});
const selectedContest = computed(
  () =>
    contestSource.value.find(
      (contest) => contest.id === selectedContestId.value,
    ) || null,
);
const selectedTrack = computed(
  () =>
    selectedContest.value?.tracks.find(
      (track) => track.id === selectedTrackId.value,
    ) || null,
);
const contestMap = computed(() => {
  const map = new Map<string, Contest>();
  for (const contest of contestSource.value) map.set(contest.id, contest);
  return map;
});
...
>(() => {
  ...
  return [...projectSettingsBindings.value]
    ...
    .reduce<WorkspaceLinkedContestResourceGroup[]>((groups, binding) => {
      ...
      const track =
        contest?.tracks.find((item) => item.id === binding.trackId) ||
        contest?.tracks[0] ||
        null;
...
const currentWorkspace = computed(() => {
  return (
    workspaceOptions.value.find(
      (item) => item.workspace.id === activeWorkspaceId.value,
    ) || null
  );
});
...
const workspaceCanManageMembers = computed(() => {
  if (me.value?.user.isPlatformAdmin) return true;

  const roles = currentWorkspace.value?.workspace.roles || [];
  if (roles.some((role) => WORKSPACE_MEMBER_MANAGE_ROLES.includes(role)))
    return true;
  return (
    currentProjectMemberRole.value === "owner" ||
    currentProjectMemberRole.value === "manager"
  );
});
...
const quickSwitchSourceProjects = computed(() => {
  const source =
    allProjects.value.length > 0 ? allProjects.value : projects.value;
...
const sortedQuickSwitchProjects = computed(() =>
  sortByUpdatedAtDesc(quickSwitchSourceProjects.value),
);
...
const activeProject = computed(() => {
  if (!highlightedProjectId.value) return null;
  const matched = projects.value.find(
    (item) => item.id === highlightedProjectId.value,
  );
  return matched || null;
});
...
const headerProjectName = computed(() => {
  if (activeProject.value?.title) return activeProject.value.title;
  if (formState.title.trim()) return formState.title.trim();
  return currentWorkspace.value?.workspace.name || "未命名项目";
});

const filteredContests = computed(() => {
  const keyword = headerSearch.value.trim();
  if (!keyword) return contests.value;
...
const projectOutlineItems = computed(
  () => projectOutlineSnapshot.value?.items || [],
);
...
function resolveResourceSourceDownloadUrl(
  resource: Resource | null | undefined,
): string {
  const projectId = String(
    resource?.projectId || activeProjectId.value || "",
  ).trim();
  const resourceId = String(resource?.id || "").trim();
  if (!projectId || !resourceId) return "";
  return buildApiUrlWithSourceRoute(
    `/projects/${projectId}/resources/${resourceId}/source`,
  );
}
...
function resolveCurrentWorkspaceRoute(): string {
  return String(route.fullPath || route.path || "").trim();
}

function buildApiUrlWithSourceRoute(path: string): string {
  const normalizedPath = String(path || "").trim();
  if (!normalizedPath) return "";

  const sourceRoute = resolveCurrentWorkspaceRoute();
  if (!sourceRoute) return endpoint(normalizedPath);

  const delimiter = normalizedPath.includes("?") ? "&" : "?";
  return endpoint(
    `${normalizedPath}${delimiter}sourceRoute=${encodeURIComponent(sourceRoute)}`,
  );
}
...
function applyWorkspaceMemberManagementSnapshot(
  snapshot: ProjectMemberManagementSnapshot,
): void {
  workspaceMembers.value = Array.isArray(snapshot.members)
    ? snapshot.members
    : [];
  workspaceInvitations.value = Array.isArray(snapshot.invitations)
...
const previewPdfUrl = computed(() => {
  const fromStatus = String(
    previewStatusPayload.value?.previewUrl || "",
  ).trim();
  if (fromStatus) return resolveApiUrl(fromStatus);
...
function isCollabResource(
  ...
  const kind = String(resource.resourceKind || "")
    .trim()
    .toLowerCase();
  if (kind === "markdown" || kind === "draw") return true;
  return (
...
function resolveCollabPurpose(
  ...
  if (!isCollabResource(resource)) return "";
...
function isWorkflowCanvasResource(
  resource: Resource | null | undefined,
): resource is Resource {
  return (
    isCollabResource(resource) &&
    resource.resourceKind === "draw" &&
    resolveCollabPurpose(resource) === "workflow"
  );
}

function resolveCollabResourceLabel(
  resource: Resource | null | undefined,
): string {
  if (!isCollabResource(resource)) return "协作内容";
...
function scheduleRealtimeProjectRefresh(): void {
  if (realtimeProjectRefreshTimer) return;
...
function clearFallbackResourceRefreshTimer(): void {
  if (!fallbackResourceRefreshTimer) return;
...
function startFallbackResourceRefreshTimer(): void {
  ...
  if (!activeProjectId.value || workspaceRealtime.connected.value) return;
...
function handleRealtimeEnvelope(message: WorkspaceRealtimeEnvelope): void {
  ...
  if (messageType === "error") {
    ...
    if (text) statusLine.value = text;
    return;
  }

  if (
    messageType === "project.resources.changed" ||
    messageType === "project.outline.changed"
  ) {
...
const mappingRows = computed<WorkspaceMappingRow[]>(() => {
  const innovationCount =
    linesToArray(formState.innovationPointsText).length +
    (chatDraft.value?.innovationPoints.length || 0);
  const routeCount =
    linesToArray(formState.techRouteStepsText).length +
    (chatDraft.value?.techRouteSteps.length || 0);
  const scoringCount =
    linesToArray(formState.scoringMappingText).length +
    (chatDraft.value?.scoringMapping.length || 0);
  const deliverableCount =
    linesToArray(formState.deliverablesText).length +
    (chatDraft.value?.deliverables.length || 0);
  const impactSignal = /社会|可持续|公益|适老|普惠|impact/i.test(
    `${formState.summary} ${formState.problemStatement}`,
  );

  const innovationScore = clamp(
    35 + innovationCount * 12 + routeCount * 6,
    10,
    98,
  );
  const marketScore = clamp(
    28 + scoringCount * 10 + (selectedResources.value.length > 0 ? 12 : 0),
    10,
    96,
  );
  const teamScore = clamp(40 + deliverableCount * 9 + routeCount * 7, 10, 100);
  const impactScore = clamp(
    impactSignal ? 74 : 18 + scoringCount * 4 + deliverableCount * 5,
    5,
    90,
  );
  ...
  return [
    {
      ...
      ability:
        innovationCount > 0
          ? linesToArray(formState.innovationPointsText)[0] ||
            "创新点已在草案中体现"
    ...
    },
    {
      ...
      score: marketScore,
      ability:
        linesToArray(formState.scoringMappingText)[0] ||
        "待补充：用户场景拆解、市场对标和商业验证数据",
      tags: ["#场景落地", "#评审关注"],
    ...
    },
    {
      ...
      score: teamScore,
      ability:
        linesToArray(formState.deliverablesText)[0] ||
        "可展示交付物框架已初步形成",
      tags: ["#团队协同", "#交付闭环"],
    ...
    },
    {
      ...
      hint: "需明确社会价值和长期影响路径",
      score: impactScore,
      ability: impactSignal
        ? "摘要中已包含社会价值路径，可继续补充量化指标"
        : "未映射：建议补充社会价值、可持续性或普惠性说明",
      tags: ["#社会价值", "#可持续"],
...
const keywordCloud = computed<WorkspaceKeyword[]>(() => {
  ...
  return words.map((label, index) => ({
    ...
    count: clamp(
      42 -
        index * 4 +
        (major.value && includesText(label, major.value) ? 6 : 0),
      8,
      56,
    ),
    active: index % 3 === 0,
...
const trendBars = computed<number[]>(() => {
  ...
  const userMessages = chatMessages.value.filter(
    (item) => item.role === "user",
  ).length;
...
const tokenBalance = computed(() => {
  ...
  if (quota) return Math.max(0, quota.aiQuotaTotal - quota.aiQuotaUsed);
  return 0;
});

const projectUploadStorageUsedBytes = computed(() => {
  return selectedResources.value.reduce(
    (sum, resource) => sum + parseFileSizeFromResource(resource),
    0,
  );
});
...
const statusCursor = computed(() => {
  const line = clamp(
    12 + chatMessages.value.length + (formState.problemStatement ? 3 : 0),
    12,
    96,
  );
...
function resetProjectSettingsState(project: Project | null) {
  ...
  try {
    ...
    Object.assign(
      ...
      createProjectCommonFormFromProject(project),
    );
...
function clearProjectSettingsAutoTimers() {
  if (!projectSettingsDraftTimer) return;
...
function markProjectSettingsAdaptationDirty(contestId: string) {
  ...
  if (
    projectSettingsDirtyAdaptationContestIds.value.includes(normalizedContestId)
  )
    return;
  projectSettingsDirtyAdaptationContestIds.value = [
    ...projectSettingsDirtyAdaptationContestIds.value,
...
function clearProjectSettingsAdaptationDirty(contestId: string) {
  ...
  if (!normalizedContestId) return;
  projectSettingsDirtyAdaptationContestIds.value =
    projectSettingsDirtyAdaptationContestIds.value.filter(
      (item) => item !== normalizedContestId,
    );
}
...
function isProjectSettingsAdaptationDirty(contestId: string): boolean {
  ...
  return projectSettingsDirtyAdaptationContestIds.value.includes(
    normalizedContestId,
  );
}

function upsertProjectSettingsAdaptationDraft(
  form: WorkspaceProjectAdaptationForm,
) {
...
function buildProjectSettingsAdaptationPatch(
  form: WorkspaceProjectAdaptationForm,
) {
  return {
...
function normalizeProjectSettingsBindings(
  ...
  for (const row of rows) {
    ...
    if (!contestId || uniqueContestIds.has(contestId)) continue;

    const contest = contestMap.value.get(contestId);
    const rawTrackId = String(row.trackId || "").trim();
    const resolvedTrackId = contest
      ? contest.tracks.find((track) => track.id === rawTrackId)?.id ||
        contest.tracks[0]?.id ||
        ""
      : rawTrackId;
...
function ensureProjectSettingsCurrentContest(preferredContestId = ""): string {
  ...
  const fallbackContestId =
    (preferred &&
      available.some((item) => item.contestId === preferred) &&
      preferred) ||
    (current &&
      available.some((item) => item.contestId === current) &&
      current) ||
    (selected &&
      available.some((item) => item.contestId === selected) &&
      selected) ||
...
function syncProjectSettingsAdaptationFormByContest(contestId: string) {
  ...
  try {
    Object.assign(
      projectSettingsAdaptation,
      cloneProjectAdaptationForm(nextDraft),
    );
  } finally {
    projectSettingsHydrating.value = false;
  }
...
function applyProjectSettingsSnapshot(
  ...
  try {
    ...
    Object.assign(
      projectSettingsCommon,
      createProjectCommonFormFromProject(snapshot.project),
    );
    ...
    const allowedContestIds = new Set(
      normalizedBindings.map((item) => item.contestId),
    );
    const keptDrafts: Record<string, WorkspaceProjectAdaptationForm> = {};
    for (const [contestId, draft] of Object.entries(
      projectSettingsAdaptationDrafts.value,
    )) {
      if (allowedContestIds.has(contestId)) keptDrafts[contestId] = draft;
    }
    ...
    if (adaptationContestId) {
      const adaptationBinding =
        projectSettingsBindingMap.value.get(adaptationContestId);
    ...
    const nextContestId = ensureProjectSettingsCurrentContest(
      preferredContestId || adaptationContestId,
    );
    if (nextContestId) selectedContestId.value = nextContestId;
    else if (!normalizedBindings.length) selectedContestId.value = "";

    const selectedBinding = projectSettingsBindingMap.value.get(nextContestId);
    if (selectedBinding) selectedTrackId.value = selectedBinding.trackId;

    syncProjectSettingsAdaptationFormByContest(nextContestId);

    projectSettingsCommonDirty.value = false;
    projectSettingsBindingsDirty.value = false;
    if (nextContestId) clearProjectSettingsAdaptationDirty(nextContestId);
  } finally {
    projectSettingsHydrating.value = false;
  }
}

function normalizeProjectSettingsDraftCachePayload(
  input: unknown,
): WorkspaceProjectSettingsDraftCache | null {
  ...
  const normalizeUi = (value: unknown): ProjectSettingsDraftUi => {
    const uiSource =
      value && typeof value === "object" && !Array.isArray(value)
        ...
        : {};
    return {
      leftSidebarCollapsed: normalizeDraftBoolean(
        uiSource.leftSidebarCollapsed,
      ),
      rightSidebarCollapsed: normalizeDraftBoolean(
        uiSource.rightSidebarCollapsed,
      ),
    };
  };
  const normalizedBindings = normalizeProjectSettingsBindings(
  ...
  const adaptationSource =
    source.adaptationDrafts &&
    typeof source.adaptationDrafts === "object" &&
    !Array.isArray(source.adaptationDrafts)
  ...
  for (const [contestId, rawValue] of Object.entries(adaptationSource)) {
    ...
    const binding = normalizedBindings.find(
      (item) => item.contestId === normalizedContestId,
    );
  ...
  const currentContestId =
    currentContestIdRaw && allowedContestIds.has(currentContestIdRaw)
      ? currentContestIdRaw
...
function applySidebarLayoutState(
  value: ProjectSettingsDraftUi | null | undefined,
): void {
...
function serializeProjectSettingsDraftCachePayload(
  payload: WorkspaceProjectSettingsDraftCache,
): string {
...
function isProjectSettingsDraftCacheEqual(
  ...
  return (
    serializeProjectSettingsDraftCachePayload(left) ===
    serializeProjectSettingsDraftCachePayload(right)
  );
}

function buildProjectSettingsDraftCachePayload(): WorkspaceProjectSettingsDraftCache {
  const currentContestId = String(
    projectSettingsCurrentContestId.value || selectedContestId.value || "",
  ).trim();
  ...
  if (currentContestId) {
    nextAdaptationDrafts[currentContestId] = cloneProjectAdaptationForm({
      ...
      trackId:
        projectSettingsBindingMap.value.get(currentContestId)?.trackId ||
        projectSettingsAdaptation.trackId,
    });
  }
...
function applyProjectSettingsDraftCachePayload(
  ...
  const hasCommonDraft = Object.values(draft.common).some(
    (value) => String(value || "").trim().length > 0,
  );
  const normalizedBindings = normalizeProjectSettingsBindings(
    Array.isArray(draft.bindings) ? draft.bindings : [],
  );
  const allowedContestIds = new Set(
    normalizedBindings.map((item) => item.contestId),
  );
  ...
  for (const [contestId, form] of Object.entries(
    draft.adaptationDrafts || {},
  )) {
    if (!allowedContestIds.has(contestId)) continue;
    const binding = normalizedBindings.find(
  ...
  const hasLayoutDraft = Boolean(
    draft.ui?.leftSidebarCollapsed || draft.ui?.rightSidebarCollapsed,
  );
  const hasDraftContent =
    hasCommonDraft ||
    normalizedBindings.length > 0 ||
    Object.keys(nextAdaptationDrafts).length > 0 ||
    hasLayoutDraft;
  ...
  try {
    ...
    const nextContestId =
      ensureProjectSettingsCurrentContest(preferredContestId);
    if (nextContestId) selectedContestId.value = nextContestId;
    ...
    projectSettingsDirtyAdaptationContestIds.value =
      Object.keys(nextAdaptationDrafts);
...
function applyProjectSettingsDraftServerRecord(
  ...
  projectSettingsDraftServerRevision.value =
    Number(record.revision || 0) || null;
...
async function fetchProjectSettingsDraftFromServer(
  projectId: string,
): Promise<WorkspaceProjectSettingsDraftCache | null> {
  const response = await $fetch<ApiResponse<ProjectSettingsDraft | null>>(
...
function pickProjectSettingsDraftForHydration(
  ...
  draft: WorkspaceProjectSettingsDraftCache | null;
  source: "local" | "server" | "";
  hasConflict: boolean;
} {
...
async function loadProjectSettings(preferredContestId = "") {
  ...
  try {
    ...
    if (activeProjectId.value !== activeId) return;
    ...
    const picked = pickProjectSettingsDraftForHydration(
      localDraft,
      serverDraft,
    );
...
async function refreshProjectSettingsDraftServerRevision(
  projectId: string,
): Promise<void> {
  try {
...
async function persistProjectSettingsDraftToServer(
  ...
  try {
    ...
    if (
      ...
      persistSeq !== projectSettingsDraftPersistSeq
    )
      return "stale";
    ...
    if (resolveApiStatusCode(error) === 409) {
      ...
      if (
        ...
        statusLine.value =
          "检测到多设备草稿冲突，已保留本地编辑。请再次保存或刷新后处理。";
      }
      return "conflict";
    }
...
async function persistProjectSettingsDraftCache() {
  if (projectSettingsHydrating.value || !activeProjectId.value) return;
  ...
  const serverResult = await persistProjectSettingsDraftToServer(
    projectId,
    payload,
    persistSeq,
  );
  ...
  if (serverResult === "conflict") return;
...
function scheduleProjectSettingsDraftPersist() {
  ...
  if (projectSettingsDraftTimer) clearTimeout(projectSettingsDraftTimer);
...
async function clearProjectSettingsDraftOnServer(
  projectId: string,
): Promise<"cleared" | "none" | "conflict" | "error"> {
  const expectedRevision = projectSettingsDraftServerRevision.value;
  if (!expectedRevision) return "none";
...
async function flushProjectSettingsSave(): Promise<boolean> {
  ...
  try {
    ...
    if (projectSettingsBindingsDirty.value)
      body.contestBindings = cloneProjectContestBindings(
        projectSettingsBindings.value,
      );
...
async function flushProjectAdaptationSave(contestId: string): Promise<boolean> {
  const normalizedContestId = String(contestId || "").trim();
  if (!activeProjectId.value || !normalizedContestId) return true;

  if (!isProjectSettingsAdaptationDirty(normalizedContestId)) return true;

  const draft = projectSettingsAdaptationDrafts.value[normalizedContestId];
  if (!draft) return true;

  const preferredContestId = String(
    projectSettingsCurrentContestId.value || selectedContestId.value || "",
  ).trim();
  projectSettingsSaveState.value = "saving";
  statusLine.value = "保存中...";
  ...
  try {
    const response = await $fetch<ApiResponse<ProjectSettingsSnapshot>>(
      endpoint(
        `/projects/${activeProjectId.value}/adaptations/${normalizedContestId}`,
      ),
      {
        method: "PATCH",
...
async function saveProjectSettingsManually() {
  ...
  const projectId = activeProjectId.value;
  if (projectId) clearProjectSettingsDraftCache(projectId);
...
function onProjectSettingsBindingsChange(
  next: WorkspaceProjectContestBindingForm[],
) {
  if (projectSettingsHydrating.value) return;
  ...
  projectSettingsAdaptationDrafts.value = keptDrafts;
  projectSettingsDirtyAdaptationContestIds.value =
    projectSettingsDirtyAdaptationContestIds.value.filter((contestId) =>
      allowedContestIds.has(contestId),
    );
  ...
  if (nextContestId) selectedContestId.value = nextContestId;
  else if (!normalized.length) selectedContestId.value = "";
...
function onProjectSettingsAdaptationChange(
  next: WorkspaceProjectAdaptationForm,
) {
  if (projectSettingsHydrating.value) return;

  const contestId = String(
    next.contestId || projectSettingsCurrentContestId.value || "",
  ).trim();
  if (!contestId) return;
...
watch(selectedContestId, (contestId) => {
  ...
  const hasExistingDraft = Boolean(
    projectSettingsAdaptationDrafts.value[normalizedContestId],
  );
...
async function loadAuthContext(): Promise<boolean> {
  try {
    ...
    const hasCurrent = resolveWorkspaceOptions(response.data).some(
      (item) => item.workspace.id === targetWorkspaceId,
    );
    if (!hasCurrent) {
      await navigateTo(
        {
...
async function loadContestCatalog() {
  try {
    const response = await $fetch<ApiResponse<Contest[]>>(
      endpoint("/contests"),
    );
...
async function loadContests() {
  ...
  try {
    ...
    for (const contest of response.data) catalogMap.set(contest.id, contest);
    ...
    if (selectedContestId.value) {
      const hit = contestCatalog.value.some(
        (contest) => contest.id === selectedContestId.value,
      );
      if (!hit) selectedContestId.value = contests.value[0]?.id || "";
    }
...
async function loadProjectResources() {
  ...
  try {
    const response = await $fetch<ApiResponse<Resource[]>>(
      endpoint(`/projects/${activeProjectId.value}/resources`),
    );
...
async function loadProjectResourceLibrary() {
  ...
  try {
    const response = await $fetch<ApiResponse<Resource[]>>(
      endpoint(`/projects/${activeProjectId.value}/resources/library`),
    );
...
async function loadProjectRecycleResources() {
  ...
  try {
    const response = await $fetch<ApiResponse<Resource[]>>(
      endpoint(`/projects/${activeProjectId.value}/resources/recycle`),
    );
...
async function loadProjectResourceShares() {
  ...
  try {
    const response = await $fetch<ApiResponse<ProjectResourceShare[]>>(
      endpoint(`/projects/${activeProjectId.value}/resources/shares`),
    );
    projectResourceShares.value = response.data.map((item) => ({
      ...item,
      shareUrl: resolveProjectResourceShareUrl(
...
function clearProjectOutlineGenerateTimer() {
  if (!projectOutlineGenerateTimer) return;
...
async function loadProjectOutline() {
  ...
  try {
    const response = await $fetch<ApiResponse<ProjectOutlineSnapshot>>(
      endpoint(`/projects/${projectId}/outline`),
      {
...
async function loadAiChangeRequests() {
  ...
  try {
    const response = await $fetch<ApiResponse<AiProjectChangeRequest[]>>(
      endpoint(`/projects/${projectId}/ai/changes`),
      {
        query: {
    ...
    aiChangeSecondConfirmIds.value = aiChangeSecondConfirmIds.value.filter(
      (item) => pendingIds.has(item),
    );
  } catch {
    if (activeProjectId.value === projectId) {
      aiChangeRequests.value = [];
...
async function loadProjectIssues() {
  ...
  try {
    const response = await $fetch<ApiResponse<ProjectIssuesBundle>>(
    ...
    if (activeProjectId.value === projectId) issueCenterLoading.value = false;
  }
}
...
async function submitIssueReport(reportId: string) {
  ...
  if (!projectId || !normalizedReportId || issueReportSubmitting.value) return;

  issueReportSubmitting.value = true;
  try {
    const response = await $fetch<
      ApiResponse<{ report: ProjectIssueReport; justSubmitted: boolean }>
    >(
      buildApiUrlWithSourceRoute(
        `/projects/${projectId}/issues/${normalizedReportId}/submit`,
      ),
      {
        method: "POST",
      },
...
async function exportIssueReport(reportId: string) {
  ...
  try {
    const response = await fetch(
      buildApiUrlWithSourceRoute(
        `/projects/${projectId}/issues/${normalizedReportId}/export`,
      ),
      {
        credentials: "include",
      },
    ...
    const report =
      projectIssueReports.value.find(
        (item) => item.id === normalizedReportId,
      ) || latestIssueReport.value;
...
function setAiChangeActing(changeId: string, active: boolean) {
  ...
  aiChangeActingIds.value = aiChangeActingIds.value.filter(
...
async function approveAiChange(change: AiProjectChangeRequest) {
  ...
  const requiresSecondConfirm =
    change.destructive && !aiChangeSecondConfirmIds.value.includes(changeId);
  ...
  try {
    ...
    await $fetch<ApiResponse<AiProjectChangeRequest>>(
      endpoint(`/projects/${projectId}/ai/changes/${changeId}/approve`),
      {
        method: "POST",
        body: payload,
      },
...
async function rejectAiChange(change: AiProjectChangeRequest) {
  ...
  try {
    await $fetch<ApiResponse<AiProjectChangeRequest>>(
      endpoint(`/projects/${projectId}/ai/changes/${changeId}/reject`),
      {
        method: "POST",
        body: {},
      },
...
async function generateProjectOutline(reason: string, silent = false) {
  ...
  try {
    const response = await $fetch<ApiResponse<ProjectOutlineSnapshot>>(
      endpoint(`/projects/${projectId}/outline/generate`),
      {
        method: "POST",
        body: {
          reason,
    ...
    if (activeProjectId.value !== projectId) return;
    projectOutlineSnapshot.value = response.data;
    if (!silent) statusLine.value = "结构大纲已更新。";
  } catch (error) {
    if (silent || activeProjectId.value !== projectId) return;
    statusLine.value = resolveApiErrorMessage(
      error,
      "结构大纲生成失败，请稍后重试。",
    );
...
async function saveWorkspaceSeatLimit(seatLimit: number) {
  ...
  workspaceSeatLimitError.value = "";
  try {
    await $fetch<ApiResponse<ProjectSeatQuota>>(
      endpoint(`/projects/${projectId}/seats`),
      {
        method: "PATCH",
        body: {
    ...
    workspaceSeatLimitError.value = resolveApiErrorMessage(
      error,
      "更新项目席位失败。",
    );
...
async function createWorkspaceInvitation(
  payload: ProjectInvitationCreatePayload,
) {
  const projectId = String(activeProjectId.value || "").trim();
  if (!projectId) {
  ...
  try {
    const response = await $fetch<
      ApiResponse<{ token: string; snapshot: ProjectMemberManagementSnapshot }>
    >(endpoint(`/projects/${projectId}/invitations`), {
      method: "POST",
      body: {
        inviteeUsername:
          String(payload.inviteeUsername || "").trim() || undefined,
        projectRole: payload.projectRole,
        expiresInDays: Math.max(
          1,
          Math.min(30, Number(payload.expiresInDays || 7)),
        ),
      },
    });
...
async function consumeJoinedProjectNotice() {
  if (!isTruthyQueryFlag(route.query.joined)) return;
...
async function patchWorkspaceMemberRole(
  payload: ProjectMemberRolePatchPayload,
) {
  const projectId = String(activeProjectId.value || "").trim();
...
async function removeWorkspaceMember(userId: string) {
  ...
  if (!projectId || !normalizedUserId) return;
  if (!workspaceCanManageMembers.value) {
...
async function revokeWorkspaceInvitation(invitationId: string) {
  ...
  if (!projectId || !normalizedInvitationId) return;
  if (!workspaceCanManageMembers.value) {
  ...
  try {
    const response = await $fetch<
      ApiResponse<ProjectMemberManagementSnapshot & { revoked?: boolean }>
    >(
      endpoint(
        `/projects/${projectId}/invitations/${normalizedInvitationId}/revoke`,
      ),
      { method: "POST" },
    );
    applyWorkspaceMemberManagementSnapshot(response.data);
    statusLine.value = response.data.revoked
      ? "项目邀请已撤销。"
      : "该项目邀请已失效，无需重复撤销。";
  } catch (error) {
    statusLine.value = resolveApiErrorMessage(
...
async function addResourceFromLibrary(resourceId: string) {
  ...
  if (!activeProjectId.value || !targetResourceId) return;
...
async function favoriteLibraryResource(resourceId: string) {
  const projectId = String(activeProjectId.value || "").trim();
  const targetResourceId = String(resourceId || "").trim();
  if (!projectId || !targetResourceId) return;

  resourceMutating.value = true;
  try {
    const response = await $fetch<
      ApiResponse<{ favorited: boolean; alreadyFavorited: boolean }>
    >(
      buildApiUrlWithSourceRoute(
        `/projects/${projectId}/resources/library/${targetResourceId}/favorite`,
      ),
      {
        method: "POST",
      },
    ...
    await refreshProjectResourceContext();
    statusLine.value = response.data.alreadyFavorited
      ? "该资料已收藏，无需重复操作。"
      : "资料已收藏。";
  } catch (error) {
    statusLine.value = resolveApiErrorMessage(
...
async function createCollabResource(kind: "markdown" | "draw") {
  ...
  try {
    const response = await $fetch<
      ApiResponse<{ resource: Resource; snapshot: CollabSnapshotPayload }>
    >(endpoint(`/projects/${projectId}/resources/collab`), {
      method: "POST",
      body: {
        kind,
      },
...
async function removeProjectResource(resourceId: string) {
  ...
  const isRemovingPreviewResource =
  ...
  try {
    await $fetch(
      endpoint(
        `/projects/${activeProjectId.value}/resources/${targetResourceId}`,
      ),
      {
        method: "DELETE",
      },
    );
    if (isRemovingPreviewResource) closeProjectResourcePreview();
    await refreshProjectResourceContext();
...
async function restoreProjectResource(resourceId: string) {
  ...
  try {
    await $fetch(
      endpoint(
        `/projects/${activeProjectId.value}/resources/${targetResourceId}/restore`,
      ),
      {
        method: "POST",
      },
...
async function purgeProjectResource(resourceId: string) {
  ...
  try {
    await $fetch(
      endpoint(
        `/projects/${activeProjectId.value}/resources/${targetResourceId}/purge`,
      ),
      {
        method: "DELETE",
      },
...
async function duplicateProjectResource(resourceId: string) {
  ...
  try {
    const response = await $fetch<ApiResponse<Resource>>(
      endpoint(
        `/projects/${activeProjectId.value}/resources/${targetResourceId}/duplicate`,
      ),
      {
        method: "POST",
      },
...
async function uploadResourcesToProject(files: File[]) {
  ...
  const validationError = validateUploadFiles(
    normalizedFiles,
    projectUploadStorageUsedBytes.value,
  );
  if (validationError) {
  ...
  try {
    const response = await $fetch<ApiResponse<{ uploadedCount?: number }>>(
      endpoint(`/projects/${activeProjectId.value}/resources/upload`),
      {
        method: "POST",
        body: formData,
      },
    ...
    const uploadedCount = Math.max(
      0,
      Number(response.data?.uploadedCount || normalizedFiles.length),
    );
...
function clearPreviewStatusPolling() {
  if (!previewStatusPollTimer) return;
...
async function fetchCollabSnapshot(
  resourceId: string,
): Promise<CollabSnapshotPayload | null> {
  const projectId = String(activeProjectId.value || "").trim();
  const targetResourceId = String(resourceId || "").trim();
  if (!projectId || !targetResourceId) return null;

  const targetResource =
    resources.value.find((item) => item.id === targetResourceId) || null;
  const resourceLabel = resolveCollabResourceLabel(targetResource);

  try {
    const response = await $fetch<ApiResponse<CollabSnapshotPayload>>(
      endpoint(`/projects/${projectId}/resources/${targetResourceId}/collab`),
    );
    return response.data;
  } catch (error) {
    statusLine.value = resolveApiErrorMessage(
      error,
      `读取${resourceLabel}失败，请稍后重试。`,
    );
    return null;
  }
}
...
async function bindCollabResource(
  ...
  const targetSnapshot =
    snapshot || (await fetchCollabSnapshot(targetResourceId));
...
async function openProjectCollabResource(
  ...
  if (options.surface === "flow") {
    ...
    if (options.openTab !== false) openFlowSignal.value += 1;
    return;
  }
  ...
  if (options.openTab !== false) openPreviewSignal.value += 1;
}

async function ensureWorkflowCanvas(
  ...
  try {
    ...
    await refreshProjectResourceContext();
    await openProjectCollabResource(
...
async function fetchResourcePreviewStatus(resourceId: string, silent = false) {
  ...
  if (!silent) previewStatusLoading.value = true;

  try {
    const response = await $fetch<ApiResponse<ResourcePreviewStatusPayload>>(
      endpoint(
        `/projects/${projectId}/resources/${targetResourceId}/preview-status`,
      ),
    );
    ...
    if (!silent) previewStatusLoading.value = false;
  }
}
...
async function openProjectResourcePreview(
  resourceId: string,
  options: OpenPreviewOptions = {},
) {
  const targetResource =
    resources.value.find((item) => item.id === resourceId) || null;
  ...
  const currentStatus = ((previewStatusPayload.value as any)?.status || "") as
    | ResourcePreviewStatus
    | "";
...
function closeProjectResourcePreview(resourceId = previewResourceId.value) {
  ...
  if (!targetResourceId || targetResourceId !== activeResourceId) return;

  const preserveFlowBinding =
    targetResourceId === String(flowResourceId.value || "").trim() &&
    activeMainTabId.value === "flow";

  closingPreviewResourceId.value = targetResourceId;
  if (!preserveFlowBinding) disposeCollabDocBinding(true);
  previewMode.value = "binary";
  previewStatusPayload.value = null;
  previewStatusLoading.value = false;
...
function downloadProjectResource(resourceId: string) {
  ...
  const targetResource = resources.value.find(
...
async function copyProjectResourceName(resourceId: string) {
  ...
  statusLine.value = copied ? "文件名已复制。" : `文件名：${targetName}`;
}

async function shareProjectResource(
  payload: ProjectResourceShareCreatePayload,
) {
  const projectId = String(activeProjectId.value || "").trim();
  const targetResourceId = String(payload?.resourceId || "").trim();
  const visibility = payload?.visibility;
  const duration = payload?.duration;
  if (!projectId || !targetResourceId || !visibility || !duration) return;

  try {
    const response = await $fetch<ApiResponse<ProjectResourceShare>>(
      endpoint(`/projects/${projectId}/resources/${targetResourceId}/shares`),
      {
        method: "POST",
        body: {
          visibility,
    ...
    const shareUrl = resolveProjectResourceShareUrl(
      String(share?.shareUrl || "").trim(),
    );
...
async function copyProjectResourceShare(shareId: string) {
  ...
  const target = projectResourceShares.value.find(
    (item) => item.id === targetShareId,
  );
  const shareUrl = resolveProjectResourceShareUrl(
    String(target?.shareUrl || "").trim(),
  );
  if (!shareUrl) {
  ...
  const copied = await copyTextToClipboard(shareUrl);
  statusLine.value = copied ? "分享链接已复制。" : `分享链接：${shareUrl}`;
}

async function revokeProjectResourceShare(shareId: string) {
  const projectId = String(activeProjectId.value || "").trim();
  const targetShareId = String(shareId || "").trim();
  if (!projectId || !targetShareId) return;

  try {
    await $fetch(
      endpoint(`/projects/${projectId}/resources/shares/${targetShareId}`),
      {
        method: "DELETE",
      },
    );
...
async function downloadPreviewSource() {
  ...
  if (import.meta.client) window.open(target, "_blank", "noopener,noreferrer");
}
...
async function reconvertProjectResourcePreview() {
  ...
  try {
    await $fetch(
      endpoint(`/projects/${projectId}/resources/${resourceId}/reconvert`),
      {
        method: "POST",
      },
    );
...
async function loadChatMessages(sessionId: string) {
  ...
  try {
    const response = await $fetch<
      ApiResponse<{ session: AiChatSession; messages: AiChatMessage[] }>
    >(
      endpoint(
        `/teams/${activeWorkspaceId.value}/chat/sessions/${sessionId}/messages`,
      ),
      {
        query: {
          projectId,
...
async function createChatSession(preferredTitle = ""): Promise<string | null> {
  const projectId = String(activeProjectId.value || "").trim();
  if (!activeWorkspaceId.value || !projectId) return null;
...
async function loadChatSessions(preferredSessionId = "") {
  ...
  try {
    ...
    const nextSession =
      chatSessions.value.find((item) => item.id === preferredSessionId) ||
...
async function startNewChatSession() {
  ...
  if (aiMode.value === "defense") modeTitle = "新建答辩会话";
  else if (aiMode.value === "auto_optimize") modeTitle = "新建自动优化会话";
  else if (aiMode.value === "issue_discovery") modeTitle = "新建寻疑发现会话";
...
function syncFormContestTrack() {
  if (!selectedContestId.value || !selectedTrackId.value) return;
...
async function runAiFilter() {
  ...
  try {
    const response = await $fetch<ApiResponse<AiContestFilterResult>>(
      endpoint("/ai/contest-filter"),
      {
        method: "POST",
        body: {
          teamId: activeWorkspaceId.value,
    ...
    normalizedInfo.value = JSON.stringify(
      response.data.normalizedFilters,
      null,
      2,
    );

    const firstContest = contests.value[0];
    if (firstContest) selectedContestId.value = firstContest.id;
...
function parseSseBlock(
  rawBlock: string,
): { eventType: string; dataText: string } | null {
...
function buildWorkspaceSystemMessage(
  ...
  if (eventType === "progress") {
    const message =
      String(data.message || "AI 处理中...").trim() || "AI 处理中...";
    return {
      role: "system",
      content: `进度：${message}`,
    };
  ...
  return {
    role: "system",
    content: summary ? `工具：${toolName} · ${summary}` : `工具：${toolName}`,
  };
}
...
async function sendWorkspaceAiMessage(pendingMessages: ChatMessage[]) {
  ...
  if (!response.body) throw new Error("未收到可读取的流式响应。");
  ...
  while (true) {
    ...
    while (true) {
      ...
      const eventType = (payload?.event ||
        parsed.eventType) as AiWorkspaceStreamEventType;
      const data = toJsonPayload(payload?.data);

      if (eventType === "progress") {
        statusLine.value = String(data.message || "AI 处理中...");
        if (data.sessionId) activeChatSessionId.value = String(data.sessionId);
        streamSystemMessages.push(
          buildWorkspaceSystemMessage("progress", data),
        );
      ...
      if (eventType === "tool") {
        ...
        if (name) statusLine.value = `AI 正在调用工具：${name}`;
        else statusLine.value = "AI 正在调用工具...";
      ...
      if (eventType === "done") {
        ...
        if (runningMode === "auto_optimize") {
          const createdCount = Array.isArray(result.proposals)
          ...
          const issues = Array.isArray(result.issues)
            ? (result.issues as ProjectIssue[])
            : [];
          if (report) {
            projectIssueReports.value = [
              report,
              ...projectIssueReports.value.filter(
          ...
          if (issues.length > 0) {
            ...
            for (const item of merged) dedupe.set(item.id, item);
  ...
  if (tail?.dataText) {
    ...
    if (payload.event === "error")
      throw new Error(
        String(toJsonPayload(payload.data).message || "工作台 AI 调用失败。"),
      );
  }

  if (runningMode === "auto_optimize") await loadAiChangeRequests();
  if (runningMode === "issue_discovery") await loadProjectIssues();
}
...
async function sendDefenseMessage(pendingMessages: ChatMessage[]) {
  ...
  while (true) {
    ...
    while (true) {
      ...
      const payload = parsed.dataText
        ...
        : null;
      const eventType = (payload?.event ||
        parsed.eventType) as AiDefenseStreamEventType;
      const data = toJsonPayload(payload?.data);
      ...
      if (eventType === "judge") {
        ...
        if (round) defenseRounds.value = [...defenseRounds.value, round];
        continue;
      }
      if (eventType === "score") {
        const scorecard = data.scorecard as AiDefenseScorecard | undefined;
        if (scorecard) defenseScorecard.value = scorecard;
        continue;
      }
      if (eventType === "delta") {
      ...
      if (eventType === "done") {
        ...
        if (Array.isArray(result.missingFields))
          chatMissingFields.value = result.missingFields.map((item) =>
            String(item),
          );
        if (result.sessionId)
          activeChatSessionId.value = String(result.sessionId);
  ...
  if (tail?.dataText) {
    ...
    if (payload.event === "error")
      throw new Error(
        String(toJsonPayload(payload.data).message || "模拟答辩失败。"),
      );
  }
}
...
async function sendChatMessage() {
  ...
  const sessionInScope = chatSessions.value.some(
    (item) => item.id === activeChatSessionId.value,
  );
  if (!sessionInScope) {
  ...
  try {
    if (aiMode.value === "defense") await sendDefenseMessage(pendingMessages);
    else await sendWorkspaceAiMessage(pendingMessages);
  } catch (error) {
    streamFailed = true;
    ...
    if (!streamFailed) await loadChatSessions(activeChatSessionId.value);
  }
}

async function submitProject(target?: {
  contestId?: string;
  trackId?: string;
}) {
  if (!activeWorkspaceId.value) {
  ...
  const contestId = String(
    target?.contestId || selectedContestId.value || "",
  ).trim();
  ...
  try {
    const contestIds =
      projectSettingsBindings.value.length > 0
        ? projectSettingsBindings.value
            .map((item) => String(item.contestId || "").trim())
            .filter(Boolean)
        : [];

    if (!contestIds.includes(contestId)) contestIds.unshift(contestId);
...
async function switchProjectFromHeader(payload: {
  projectId: string;
  workspaceId: string;
}) {
  const target = quickSwitchSourceProjects.value.find(
    (item) => item.id === payload.projectId,
  );
  if (target) statusLine.value = `已定位项目：${target.title}`;
...
async function openFinalReviewFromHeader() {
  ...
  if (opened) statusLine.value = "已打开流程画布，可按流程继续推进终审。";
}
...
async function openFlowFromLeftSidebar() {
  const opened = await ensureWorkflowCanvas();
  if (opened) statusLine.value = "已打开流程画布，可继续协作梳理项目流程。";
}
...
onMounted(async () => {
  ...
  unsubscribeRealtimeMessages = workspaceRealtime.onMessage(
...
watch(routeWorkspaceId, async (value, previous) => {
  ...
  const hasCurrent = resolveWorkspaceOptions(me.value).some(
    (item) => item.workspace.id === value,
  );
  if (!hasCurrent) {
    await navigateTo(
      {
  ...
  if (activeWorkspaceId.value !== value) activeWorkspaceId.value = value;
  workspaceRealtime.subscribeWorkspace(value);
...
watch(activeProjectId, async (next, previous) => {
  ...
  if (next) workspaceRealtime.subscribeProject(next);
  await refreshProjectResourceContext();
  if (!next) {
    disposeCollabDocBinding(true);
...
watch(
  ...
  (nextResources) => {
    const workflowResource =
      nextResources.find((item) => isWorkflowCanvasResource(item)) || null;
    if (workflowResource && !flowResourceId.value)
      flowResourceId.value = workflowResource.id;

    if (
      flowResourceId.value &&
      !nextResources.some((item) => item.id === flowResourceId.value)
    ) {
      const shouldDispose =
        collabBindingResourceId.value === flowResourceId.value &&
        activeMainTabId.value === "flow";
      flowResourceId.value = "";
      if (shouldDispose) disposeCollabDocBinding(true);
    }
  },
  { deep: true },
);
...
watch(activeMainTabId, async (next, previous) => {
  ...
  if (next === "flow") {
    ...
    if (!targetResourceId) return;
    if (collabBindingResourceId.value === targetResourceId) return;
  ...
  if (!next.startsWith("resource:")) return;
  ...
  const targetResource =
    resources.value.find((item) => item.id === targetResourceId) || null;
  if (!isCollabResource(targetResource)) return;
...
watch(
  ...
  ([nextLeft, nextRight], [prevLeft, prevRight]) => {
    if (nextLeft === prevLeft && nextRight === prevRight) return;
    if (sidebarLayoutHydrating.value || !activeProjectId.value) return;
    scheduleProjectSettingsDraftPersist();
  },
);
...
watch(
  () => workspaceRealtime.connected.value,
  () => {
    syncFallbackResourceRefreshTimer();
  },
);
...
<template>
  <div
    class="workspace-shell text-slate-800 bg-white h-full min-h-0 overflow-hidden"
  >
    <WorkspaceHeader
    ...
    <main
      class="workspace-layout flex flex-1 min-h-0 items-stretch overflow-hidden xl:flex-row"
    >
      <div
        v-if="!leftSidebarCollapsed"
        class="workspace-side-anchor workspace-side-anchor--left"
      >
        <WorkspaceLeftSidebar
          ...
          @favorite-library-resource="favoriteLibraryResource"
        ...
        />
        <div
          class="workspace-side-handle workspace-side-handle--left workspace-side-handle--left-expanded"
        >
          <button
      ...
      <div
        v-else
        class="workspace-side-handle workspace-side-handle--left workspace-side-handle--left-collapsed"
      >
        <button
          class="workspace-side-toggle"
          type="button"
      ...
      <WorkspaceMainPanel
        ...
        :workspace-member-role-updating-user-id="
      ...
      <div
        ...
        class="workspace-side-anchor workspace-side-anchor--right"
      >
        <div
          class="workspace-side-handle workspace-side-handle--right workspace-side-handle--right-expanded"
        >
          <button
            class="workspace-side-toggle"
            type="button"
      ...
      <div
        v-else
        class="workspace-side-handle workspace-side-handle--right workspace-side-handle--right-collapsed"
      >
        <button
          class="workspace-side-toggle"
          type="button"

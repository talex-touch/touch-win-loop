...
<template>
  <section class="px-4 py-4 border border-slate-200 rounded-xl bg-white">
    <div class="flex flex-wrap gap-2 items-center justify-between">
      <div>
        <h3 class="text-base text-slate-900 font-bold flex gap-2 items-center">
      ...
      <NuxtLink
        ...
        to="/admin"
        class="text-[13px] text-slate-700 font-semibold px-3 py-1.5 border border-slate-200 rounded-md hover:bg-slate-50"
      >
        进入平台管理
      </NuxtLink>
    </div>

    <div class="mt-4 gap-2.5 grid md:grid-cols-2 xl:grid-cols-3">
      <NuxtLink
        v-for="item in portalCards"
        :key="item.id"
        :to="item.to"
        class="px-3.5 py-3 border border-slate-100 rounded-lg transition-colors hover:border-blue-200 hover:bg-blue-50/40"
      >
        ...
        <p class="text-[13px] text-slate-600 leading-5 mt-1.5">
          {{ item.desc }}
        </p>
      </NuxtLink>
    </div>

    <div class="mt-4 gap-3 grid xl:grid-cols-2">
      <article class="px-3.5 py-3 border border-slate-100 rounded-lg">
        ...
        <div v-if="platformLoading" class="mt-3 space-y-2">
          <div
            ...
            class="border border-slate-100 rounded-md bg-slate-100 h-8 animate-pulse"
          />
        </div>
        ...
        <div v-else class="mt-3 space-y-2">
          <NuxtLink
            ...
            :key="item.id"
            :to="`/contests/${item.id}`"
            class="px-3 py-2 border border-slate-100 rounded-md flex items-center justify-between hover:border-slate-300"
          >
      ...
      <article class="px-3.5 py-3 border border-slate-100 rounded-lg">
        ...
        <div v-else class="mt-3 flex flex-wrap gap-2">
          <span
            ...
            :key="permission"
            class="text-[11px] text-emerald-700 font-semibold px-2 py-1 rounded-md bg-emerald-50"
          >
            {{ permission }}
          </span>
        </div>

...
function initAurora() {
  ...
  const resize = () => {
    ...
    const height = Math.max(container.parentElement?.offsetHeight || container.offsetHeight || window.innerHeight, 300)
  ...
  const update = (time: number) => {
    ...
    if (program) {
      program.uniforms.uTime.value = (props.time ?? time * 0.01) * (props.speed ?? 1.0) * 0.1
      ...
      program.uniforms.uColorStops.value = (props.colorStops ?? ['#27FF64', '#7cff67', '#27FF64']).map((hex) => {
        const color = new Color(hex)
        return [color.r, color.g, color.b]
      })
...
watch(() => [props.colorStops, props.amplitude, props.blend, props.speed, props.intensity], () => {
  cleanup?.()
  cleanup = initAurora()
}, { deep: true })

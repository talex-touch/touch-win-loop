...
const hasUploadPanelContent = computed(() => uploadActivityItems.value.length > 0)
...
<template>
  <div class="workspace-left-panel__feature">
    <div class="workspace-left-panel__body no-scrollbar">
      <section
        data-testid="workspace-left-panel-upload-module"
        class="workspace-upload-panel"
      >
        <div class="workspace-upload-panel__header">
          <div>
            <h3>上传管理</h3>
            <div class="workspace-upload-panel__summary">
              <span>{{ uploadTrayText }}</span>
              <template v-if="uploadTrayMetaText">
                · <span>{{ uploadTrayMetaText }}</span>
              </template>
            </div>
          </div>
          <div class="workspace-upload-panel__actions">
            <button
              v-if="pausableUploadItems.length > 0"
              class="workspace-upload-panel__action"
              type="button"
        ...
        <div class="workspace-upload-panel__body">
          <div v-if="myUploadItems.length > 0" class="workspace-upload-panel__group">
            <div class="workspace-upload-panel__group-title">
              我的上传
            </div>
            <div
          ...
          <div v-if="teamUploadingItems.length > 0" class="workspace-upload-panel__group">
            <div class="workspace-upload-panel__group-title">
              团队上传中
            </div>
            <div
          ...
          <div v-if="recentCompletedItems.length > 0" class="workspace-upload-panel__group">
            <div class="workspace-upload-panel__group-title">
              最近完成
            </div>
            <div
          ...
          <div v-if="pendingUploadItems.length > 0" class="workspace-upload-panel__group">
            <div class="workspace-upload-panel__group-title">
              失败 / 待处理
            </div>
            <div
          ...
          <div
            v-if="!hasUploadPanelContent"
            class="workspace-upload-panel__empty"
          >
...
.workspace-upload-panel__summary {
  margin-top: 4px;
  color: #74849c;
  font-size: 12px;
  line-height: 1.5;
...
.workspace-upload-panel__actions {
  display: inline-flex;
  flex-wrap: wrap;
...
.workspace-upload-panel__action {
  height: 28px;
  padding: 0 10px;
...
.workspace-upload-panel__action:hover {
  background: #eef5ff;
}

.workspace-upload-panel__body {
  flex: 1 1 auto;
  min-height: 0;
  overflow-y: auto;
  padding-right: 2px;
...
.workspace-upload-panel__group {
  margin-top: 12px;
}

.workspace-upload-panel__group:first-child {
  margin-top: 0;
}

.workspace-upload-panel__group-title {
  margin-bottom: 8px;
  color: #51627e;
  font-size: var(--wl-text-caption);
  font-weight: 700;
...
.workspace-upload-panel__group-title + .workspace-upload-task-row {
  border-top: none;
}

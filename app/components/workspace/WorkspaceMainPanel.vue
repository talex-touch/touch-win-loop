...
const projectSettingsDisplayHasOverride = computed(() => {
  return Boolean(
    String(props.projectSettingsCommon.icon || '').trim()
    && String(props.projectSettingsCommon.accentColor || '').trim(),
  )
})

const projectSettingsDisplayPreview = computed(() => {
  const seed = `${String(props.activeProject?.id || 'project').trim()}:${String(props.activeProject?.title || '').trim()}`
  const configuredDisplay = normalizeProjectDisplayConfig({
    icon: props.projectSettingsCommon.icon,
    accentColor: props.projectSettingsCommon.accentColor,
  })
  const display = resolveProjectDisplayConfig(configuredDisplay || props.activeProject?.display || null, seed)
  const accent = getProjectDisplayAccent(display.accentColor)
  const title = String(props.projectSettingsCommon.title || props.activeProject?.title || '').trim() || '未命名项目'
  const summary = String(props.projectSettingsCommon.summary || props.activeProject?.summary || '').trim() || '项目卡会使用当前图标与颜色方案展示。'
...
const projectSettingsCustomAccentValue = computed(() => {
  const normalized = normalizeProjectDisplayAccentColor(
    props.projectSettingsCommon.accentColor || projectSettingsDisplayPreview.value.accent.value,
  )
...
const projectSettingsUsingCustomAccent = computed(() => {
  const normalized = normalizeProjectDisplayAccentColor(props.projectSettingsCommon.accentColor)
                    ...
                    <div>
                      ...
                      <div class="gap-2 grid grid-cols-2 sm:grid-cols-4">
                        <button
                          ...
                          class="px-3 py-2.5 border rounded-xl bg-white transition-all"
                    ...
                    <div>
                      ...
                      <div class="gap-2 grid grid-cols-2 sm:grid-cols-4">
                        <button
                          ...
                          class="px-3 py-2 border rounded-xl bg-white transition-all"
                      ...
                      <div class="mt-3 px-3 py-2 border border-slate-200 rounded-xl bg-white flex flex-wrap gap-3 items-center">
                        <label class="flex gap-2 items-center">
                          ...
                          <input
                            class="border border-slate-200 rounded bg-transparent h-8 w-10 cursor-pointer"
